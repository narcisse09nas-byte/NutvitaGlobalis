...
export async function POST(request: Request) {
  try {
    ...
    const { data: payment, error: readError } = await admin.from("payments").select("*").eq("id", String(body.payment_id)).eq("provider", "manual").maybeSingle();
    ...
    if (body.action === "approve") {
      ...
      const review = await admin.from("payments").update({ manual_reviewed_by: user.id, manual_reviewed_at: new Date().toISOString(), manual_review_notes: String(body.notes || "") }).eq("id", payment.id);
      if (review.error) return NextResponse.json({ message: review.error.message }, { status: 500 });
