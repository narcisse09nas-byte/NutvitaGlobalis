...
export async function POST(request:Request){
  ...
  if(menuError||!menus||menus.length!==ids.length||menus.some((menu:any)=>menu.city!==body.city))return NextResponse.json({message:"Un menu sÃ©lectionnÃ© nâ€™est plus disponible dans cette ville."},{status:400});
  ...
  if(error||!order)return NextResponse.json({message:error?.message||"Commande non crÃ©Ã©e."},{status:400});
...
export async function PATCH(request:Request){
  ...
  if(!current||current.status!=="quoted")return NextResponse.json({message:"Cette commande ne peut pas Ãªtre confirmÃ©e."},{status:409});
