...
const appDefinitions=[
 {key:"nutritrack",Icon:HeartPulse,badge:"PHARE"},
 {key:"survey",Icon:ClipboardList},
 {key:"project_management",Icon:BarChart3},
] as const;
...
export default async function SupportApplicationsPage(){
 ...
 return <main className="overflow-hidden bg-[#fbfcfa] text-forest">
  ...
  <section id="applications" className="section scroll-mt-24"><div className="container-site grid items-stretch gap-6 lg:grid-cols-3">{appDefinitions.map((definition,index)=>{const item=apps[index],Icon=definition.Icon;return <article key={definition.key} className="group flex min-h-[590px] flex-col rounded-[2rem] border bg-white p-7 shadow-soft transition hover:-translate-y-1 hover:shadow-xl md:p-8"><div className="flex items-start gap-5"><span className="grid h-16 w-16 shrink-0 place-items-center rounded-2xl bg-[#eaf5eb] text-leaf"><Icon className="h-8"/></span><div className="flex-1"><div className="flex items-start justify-between gap-3"><h2 className="text-2xl font-black leading-7">{item.title}</h2>{"badge" in definition&&<span className="rounded-full bg-orange/10 px-3 py-1 text-[10px] font-black uppercase tracking-widest text-orange">{definition.badge}</span>}</div></div></div><p className="mt-5 text-sm leading-7 text-slate-600">{item.text}</p><ul className="mt-6 grid gap-3 text-sm">{(item.items||[]).map(feature=><li key={feature} className="flex gap-3"><CheckCircle2 className="h-5 shrink-0 text-leaf"/><span>{feature}</span></li>)}</ul><Link href={localizedPath(locale,definition.key==="nutritrack"?"/applications-support/nutritrack":definition.key==="survey"?"/applications-support/sansurvey":"/applications-support/promanage")} className="mt-auto inline-flex min-h-12 items-center justify-center rounded-full bg-orange px-6 font-black text-white shadow-lg shadow-orange/15 transition group-hover:brightness-95">{t("Accéder à l’application","Access the application")}<ArrowRight className="ml-3 h-4 transition group-hover:translate-x-1"/></Link></article>})}</div></section>
