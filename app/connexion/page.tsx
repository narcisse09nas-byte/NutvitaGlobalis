...
export default async function Page({ searchParams }: { searchParams: Promise<{ redirect?: string; identifiant?: string }> }) {
  ...
  const requestedService=target.includes("/api/access/open")?new URL(target,"https://nutvitaglobalis.local").searchParams.get("service"):null;
  const service=platformServices.find(item=>item.key===requestedService);
  const serviceLabel=service?(locale==="en"?service.titleEn||service.title:service.title):undefined;
