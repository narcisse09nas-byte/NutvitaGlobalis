...
export default async function Contact() {
  ...
  const items: ContactItem[] = [
    ...
    { Icon: MapPinIcon, label: "Adresse", value: siteConfig.address },
  ];
  return <>{page && <><ManagedPageHero initial={page} /><ManagedPageSections initial={page} /></>}<section className="section"><div className="container-site grid gap-10 lg:grid-cols-[.8fr_1.2fr]"><div><h2 className="text-3xl font-black">Coordonnees</h2><div className="mt-8 grid gap-4">{items.map(({ Icon, label, value, href }) => {
  ...
  })}</div><Link href={whatsappUrl()} target="_blank" className="btn-primary mt-6">Ecrire sur WhatsApp</Link></div><ContactForm /></div></section></>;
}
