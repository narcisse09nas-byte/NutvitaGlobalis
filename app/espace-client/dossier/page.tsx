...
export default async function NutritionRecordPage() {
  ...
  const [{ data: anthropometry }, { data: biology }, { data: food }, { data: consultations }] = await Promise.all([
  ...
  return <ClientShell email={user.email || ""}><div className="mb-7"><h1 className="text-3xl font-black">Dossier nutritionnel</h1><p className="mt-2 text-slate-500">Vos historiques anthropometriques, biologiques, alimentaires et cliniques.</p></div><NutritionRecord clientId={user.id} anthropometry={anthropometry || []} biology={biology || []} food={food || []} consultations={consultations || []} /></ClientShell>;
}
