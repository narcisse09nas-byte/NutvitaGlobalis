...
export default async function ClientHome({ searchParams }: { searchParams: Promise<{ paiement?: string; activation?: string }> }) {
  ...
  return <ClientShell email={user.email || ""}><div className="grid gap-8">
    ...
    <Section title="Mes abonnements, packs et beneficiaires">
      {subscriptions?.map((item:any) => <Link key={item.id} href={item.subscription_plans?.service_type==="child_growth"?"/espace-client/croissance-enfant":"/espace-client/dossier"} className="flex flex-wrap items-center justify-between gap-3 rounded-xl bg-slate-50 p-4"><div><b>{item.subscription_plans?.name||item.plan_id}</b><p className="mt-1 text-sm text-slate-500">{item.children?.full_name||"Moi-meme"} · {item.expires_at?`jusqu au ${new Date(item.expires_at).toLocaleDateString("fr-FR")}`:"activation en attente"}</p></div><span className="rounded-full bg-mint px-3 py-1 text-xs font-bold uppercase text-forest">{item.status}</span></Link>)}
      {bookings?.filter((item:any)=>!["cancelled","refunded"].includes(item.status)).map((item:any) => {
        const expired=item.access_expires_at&&+new Date(item.access_expires_at)<=Date.now();
        return <Link key={`pack-${item.id}`} href="/espace-client/consultations" className="flex flex-wrap items-center justify-between gap-3 rounded-xl bg-orange/10 p-4"><div><b>Pack {item.teleconseils?.name||"teleconseil"}</b><p className="mt-1 text-sm text-slate-500">Moi-meme · {item.access_expires_at?`jusqu au ${new Date(item.access_expires_at).toLocaleDateString("fr-FR")}`:"activation en attente"}</p></div><span className="rounded-full bg-white px-3 py-1 text-xs font-bold uppercase text-forest">{expired?"expire":item.status==="pending"?"en attente":"actif"}</span></Link>;
      })}
