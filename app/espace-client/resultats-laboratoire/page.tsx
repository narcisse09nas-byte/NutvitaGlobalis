...
export default async function LaboratoryResultsPage() {
  ...
  return <ClientShell email={user.email || ""}>
    <div className="mb-7"><h1 className="text-3xl font-black">Résultats de laboratoire</h1><p className="mt-2 text-slate-500">Enregistrez un résultat uniquement lorsque vous ouvrez cette rubrique.</p></div>
    <PrescriptionResultsCenter view="results" clientId={user.id} partnerId={partnerId} consultations={consultations || []} initialResults={results || []} initialConsents={consents || []}/>
  </ClientShell>;
}
