...
export default async function ServicesPage({ searchParams }: { searchParams: Promise<{ categorie?: string }> }) {
  ...
  const [{ data: plans }, { data: packs }, { data: courses }, { data: children }, tax] = await Promise.all([
  ...
  return <ClientShell email={user.email || ""}><div className="mb-7"><h1 className="text-3xl font-black">Services disponibles</h1><p className="mt-2 text-slate-500">Choisissez un service depuis votre espace client, puis selectionnez Mobile Money manuel, virement bancaire ou une solution automatique disponible.</p></div><ServiceCatalog plans={plans || []} packs={packs || []} courses={courses || []} children={children || []} taxRate={Number(tax.rate)} initialFilter={params.categorie || "all"} /></ClientShell>;
}
