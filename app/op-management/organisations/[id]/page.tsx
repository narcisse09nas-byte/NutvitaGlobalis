...
export default async function OrganizationDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <PPMShell name={name} breadcrumbs={[{ href: "/op-management", label: "Vue d'ensemble" }, { href: "/op-management/organisations", label: "Organisations" }, { href: `/op-management/organisations/${id}`, label: organization.name }]}>
    ...
    <div className="mb-7 grid gap-4 sm:grid-cols-3">
      <div className="rounded-2xl border bg-white p-5"><p className="text-xs font-bold uppercase text-slate-400">Portefeuilles</p><b className="mt-2 block text-2xl text-forest">{portfolios.length}</b></div>
      <div className="rounded-2xl border bg-white p-5"><p className="text-xs font-bold uppercase text-slate-400">Budget consolide</p><b className="mt-2 block text-2xl text-forest">{totalBudget ? totalBudget.toLocaleString("fr-FR") : "—"}</b></div>
      <div className="rounded-2xl border bg-white p-5"><p className="text-xs font-bold uppercase text-slate-400">Pays</p><b className="mt-2 block text-2xl text-forest">{organization.country || "—"}</b></div>
    </div>
