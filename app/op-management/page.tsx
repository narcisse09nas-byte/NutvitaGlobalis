...
export default async function OperationsManagementPage() {
  ...
  return <PPMShell name={name} locale={locale} breadcrumbs={[{ href: "/op-management", label: bc(locale, "overview") }]}>
    <div className="mb-7"><h1 className="text-3xl font-black text-forest">{bc(locale, "overview")}</h1><p className="mt-2 text-slate-500">{en ? "Portfolios, programs, projects and overall organization performance." : "Portefeuilles, programmes, projets et performance globale de l'organisation."}</p></div>

    {counts.organizations === 0 ? <PPMOverviewEmptyState /> : <div className="grid gap-6">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Link href="/op-management/organisations" className="rounded-2xl border bg-white p-5 transition hover:-translate-y-0.5 hover:shadow-soft"><span className="grid h-10 w-10 place-items-center rounded-xl bg-mint text-leaf"><BuildingOffice2Icon className="h-5" /></span><b className="mt-3 block text-2xl text-forest">{counts.organizations}</b><span className="text-xs font-bold uppercase text-slate-400">{en ? "Organization(s)" : "Organisation(s)"}</span></Link>
        <Link href="/op-management/portefeuilles" className="rounded-2xl border bg-white p-5 transition hover:-translate-y-0.5 hover:shadow-soft"><span className="grid h-10 w-10 place-items-center rounded-xl bg-mint text-leaf"><Squares2X2Icon className="h-5" /></span><b className="mt-3 block text-2xl text-forest">{counts.portfolios}</b><span className="text-xs font-bold uppercase text-slate-400">{en ? "Portfolio(s)" : "Portefeuille(s)"}</span></Link>
        <Link href="/op-management/programmes" className="rounded-2xl border bg-white p-5 transition hover:-translate-y-0.5 hover:shadow-soft"><span className="grid h-10 w-10 place-items-center rounded-xl bg-mint text-leaf"><BriefcaseIcon className="h-5" /></span><b className="mt-3 block text-2xl text-forest">{counts.programs}</b><span className="text-xs font-bold uppercase text-slate-400">{en ? "Program(s)" : "Programme(s)"}</span></Link>
        <Link href="/op-management/projets" className="rounded-2xl border bg-white p-5 transition hover:-translate-y-0.5 hover:shadow-soft"><span className="grid h-10 w-10 place-items-center rounded-xl bg-mint text-leaf"><ClipboardDocumentListIcon className="h-5" /></span><b className="mt-3 block text-2xl text-forest">{counts.projects}</b><span className="text-xs font-bold uppercase text-slate-400">{en ? "Project(s)" : "Projet(s)"}</span></Link>
      </div>

      <section className="rounded-2xl border bg-white p-6">
        <div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-xl font-black text-forest">{bc(locale, "organizations")}</h2><Link href="/op-management/organisations" className="text-sm font-black text-leaf">{en ? "View all →" : "Voir tout →"}</Link></div>
