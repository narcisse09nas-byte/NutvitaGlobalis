...
export default async function PortfolioDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <PPMShell name={name} locale={locale} breadcrumbs={[{ href: "/op-management", label: bc(locale, "overview") }, { href: "/op-management/portefeuilles", label: bc(locale, "portfolios") }, { href: `/op-management/portefeuilles/${id}`, label: portfolio.name }]}>
    ...
    <div className="mb-7 grid gap-4 sm:grid-cols-3">
      <div className="rounded-2xl border bg-white p-5"><p className="text-xs font-bold uppercase text-slate-400">{bc(locale, "programs")}</p><b className="mt-2 block text-2xl text-forest">{programs.length}</b></div>
      ...
      <div className="rounded-2xl border bg-white p-5"><p className="text-xs font-bold uppercase text-slate-400">{en ? "Manager" : "Responsable"}</p><b className="mt-2 block text-2xl text-forest">{portfolio.manager_name || "—"}</b></div>
    </div>
