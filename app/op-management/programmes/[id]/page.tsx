...
export default async function ProgramDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <PPMShell name={name} breadcrumbs={[{ href: "/op-management", label: "Vue d'ensemble" }, { href: "/op-management/programmes", label: "Programmes" }, { href: `/op-management/programmes/${id}`, label: program.name }]}>
    ...
    <div className="mb-7 grid gap-4 sm:grid-cols-3">
      ...
      <div className="rounded-2xl border bg-white p-5"><p className="text-xs font-bold uppercase text-slate-400">Progression</p><b className="mt-2 block text-2xl text-forest">{program.progress_percent != null ? `${program.progress_percent}%` : "—"}</b></div>
      <div className="rounded-2xl border bg-white p-5"><p className="text-xs font-bold uppercase text-slate-400">Responsable</p><b className="mt-2 block text-2xl text-forest">{program.manager_name || "—"}</b></div>
    </div>
    {(program.expected_results || program.target_population || program.intervention_area || program.donors?.length || program.partners?.length) && <div className="mb-7 grid gap-4 rounded-2xl border bg-white p-6 sm:grid-cols-2">
      {program.expected_results && <div className="sm:col-span-2"><p className="text-xs font-bold uppercase text-slate-400">Resultats attendus</p><p className="mt-1 text-sm text-slate-600">{program.expected_results}</p></div>}
      {program.target_population && <div><p className="text-xs font-bold uppercase text-slate-400">Population cible</p><p className="mt-1 text-sm text-slate-600">{program.target_population}</p></div>}
      {program.intervention_area && <div><p className="text-xs font-bold uppercase text-slate-400">Zone d&apos;intervention</p><p className="mt-1 text-sm text-slate-600">{program.intervention_area}</p></div>}
      {!!program.donors?.length && <div><p className="text-xs font-bold uppercase text-slate-400">Bailleurs</p><p className="mt-1 text-sm text-slate-600">{program.donors.join(", ")}</p></div>}
      {!!program.partners?.length && <div><p className="text-xs font-bold uppercase text-slate-400">Partenaires</p><p className="mt-1 text-sm text-slate-600">{program.partners.join(", ")}</p></div>}
    </div>}
