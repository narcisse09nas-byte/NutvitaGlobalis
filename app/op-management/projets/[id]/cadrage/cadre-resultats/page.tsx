...
import { getProject, listIndicators, listResultChain } from "@/lib/ppm/queries";
...
export default async function CadrageResultsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!project) notFound();
  const [resultChain, indicators] = await Promise.all([listResultChain(supabase, id), listIndicators(supabase, id)]);
