...
import { getProject, listActivities, listDeliverables, listDocuments, listWbsNodes } from "@/lib/ppm/queries";
...
export default async function ReportingLivrablesDocumentsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!project) notFound();
  const [deliverables, documents, wbsNodes, activities] = await Promise.all([
  ...
  return <PPMShell name={name} breadcrumbs={[{ href: "/op-management", label: "Vue d'ensemble" }, { href: "/op-management/projets", label: "Projets" }, { href: `/op-management/projets/${id}`, label: project.name }, { href: `/op-management/projets/${id}/reporting/livrables-documents`, label: "Reporting" }]}>
    <ProjectShell project={project}>
      <div className="grid gap-8">
        ...
        <DeliverableManager projectId={id} initial={deliverables} wbsNodes={wbsNodes} activities={activities} />
        <DocumentRegister projectId={id} initial={documents} deliverables={deliverables} />
      </div>
    </ProjectShell>
  </PPMShell>;
}
