...
import { getProject, listEvaluations, listFeedbackEntries, listFeedbackFollowups, listIndicators, listLessonsLearned, listMealEntries } from "@/lib/ppm/queries";
...
export default async function SuiviControleMealPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!project) notFound();
  const [indicators, mealEntries, evaluations, feedbackEntries, lessonsLearned, feedbackFollowups] = await Promise.all([
    listIndicators(supabase, id), listMealEntries(supabase, id), listEvaluations(supabase, id),
    listFeedbackEntries(supabase, id), listLessonsLearned(supabase, id), listFeedbackFollowups(supabase, id),
  ]);
  ...
  return <PPMShell name={name} breadcrumbs={[{ href: "/op-management", label: "Vue d'ensemble" }, { href: "/op-management/projets", label: "Projets" }, { href: `/op-management/projets/${id}`, label: project.name }, { href: `/op-management/projets/${id}/suivi-controle/meal`, label: "Suivi & controle" }]}>
    <ProjectShell project={project}>
      <div className="grid gap-8">
        ...
        <AccountabilityManager projectId={id} initialFeedback={feedbackEntries} initialLessons={lessonsLearned} initialFollowups={feedbackFollowups} />
      </div>
    </ProjectShell>
  </PPMShell>;
}
