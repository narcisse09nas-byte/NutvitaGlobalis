...
export default async function Page({ searchParams }: { searchParams: Promise<{ from?: string; to?: string; status?: string }> }) {
  ...
  const eligible = (data || []).filter((x: any) => ["earning", "adjustment"].includes(x.entry_type) && ["approved", "paid"].includes(x.status));
  const generatedOnsite = eligible.filter((x: any) => String(x.description || "").toLowerCase().includes("sur site")).reduce((s: number, x: any) => s + Number(x.amount || 0) * 2, 0);
  const generatedPlatform = eligible.filter((x: any) => !String(x.description || "").toLowerCase().includes("sur site")).reduce((s: number, x: any) => s + Number(x.amount || 0) * 2, 0);
  ...
  return <PartnerShell email={user.email || ""}>
    ...
    <div className="mb-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4"><Metric label="Revenus générés sur site" value={generatedOnsite} /><Metric label="Revenus générés via la plateforme" value={generatedPlatform} /><Metric label="Déjà payé par NutVitaGlobalis" value={paid} /><Metric label="Crédit restant dû" value={credit} /></div>
