...
export default async function HealthTracking() {
  ...
  return <>{page && <ManagedPageHero initial={page} />}<MedicalDisclaimer />
    <section className="bg-forest py-10 text-white"><div className="container-site flex flex-wrap items-center justify-between gap-5"><p className="text-2xl font-black text-orange">{english ? "Paid service" : "Service payant"}</p><div className="flex flex-wrap gap-3"><Link href="/inscription" className="btn-primary bg-orange">{english ? "Create my account" : "Creer mon compte"}</Link><Link href="/espace-client/abonnement" className="btn-secondary border-white text-white">{english ? "Activate monitoring" : "Activer le suivi"}</Link></div></div></section>
    ...
    <section className="section bg-mint"><div className="container-site grid gap-6 text-center md:grid-cols-2"><Offer title={english ? "Autonomous Health Monitoring" : "Suivi Sante Autonome"} text={english ? "Paid service. Payment is not available yet." : "Service payant. Le paiement n est pas encore disponible."} href="/espace-client/abonnement" action={english ? "Activate" : "Activer"} /><Offer title={english ? "Child Growth" : "Croissance Enfant"} text={english ? "Paid service per child. Payment is not available yet." : "Service payant par enfant. Le paiement n est pas encore disponible."} href="/espace-client/croissance-enfant" action={english ? "Add a child" : "Ajouter un enfant"} /></div></section>
  </>;
}
