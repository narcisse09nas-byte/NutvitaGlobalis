...
export async function POST(request: Request, { params }: { params: Promise<{ sessionId: string }> }) {
  ...
  const { error } = await supabase.from("academy_live_attendance").insert({ session_id: sessionId, user_id: auth.user.id, full_name: profile?.full_name ?? "Participant" });
...
export async function DELETE(request: Request, { params }: { params: Promise<{ sessionId: string }> }) {
  ...
  const { data: auth } = await supabase.auth.getUser();
  if (!auth.user) return Response.json({ error: "Authentification requise." }, { status: 401 });
  const { error } = await supabase.from("academy_live_attendance").update({ left_at: new Date().toISOString() }).eq("session_id", sessionId).eq("user_id", auth.user.id).is("left_at", null);
  if (error) return Response.json({ error: error.message }, { status: 403 });
