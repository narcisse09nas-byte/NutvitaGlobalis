              ...
              : bookingPolicy.status === "awaiting_conduct_review"
                ...
                : bookingPolicy.status === "certified"
                  ...
                  : bookingPolicy.status === "attempts_exhausted"
                    ? text("Les trois tentatives autorisées sont épuisées.", "All authorized attempts have been used.")
                    : text(
                        `Tentative ${bookingPolicy.attemptNumber}/${exam.maxAttempts <= 0 ? "âˆž" : exam.maxAttempts} : choisissez un créneau à partir du ${earliestSlot?.toLocaleDateString(locale === "fr" ? "fr-FR" : "en-US")}.`,
              ...
              {slots.map((slot) => {
                ...
                return (
                  <div key={slot.id} className="rounded-2xl border border-slate-200 p-4">
                    ...
                    >
                      {!eligible ? text("Terminer le parcours", "Complete the learning path") : text(`Réserver la tentative ${bookingPolicy?.attemptNumber ?? 1}/${exam.maxAttempts <= 0 ? "âˆž" : exam.maxAttempts}`, `Book attempt ${bookingPolicy?.attemptNumber ?? 1}/${exam.maxAttempts <= 0 ? "âˆž" : exam.maxAttempts}`)}
                    </button>
                  </div>
                );
              })}
