...
export function ExamScheduling() {
  ...
  const { allAttempts: examAttempts } = useExam();
  ...
  const exams = [...examDefinitions.map((exam) => ({ exam, questionPool: examQuestionBank, course: getLocalCourseBySlug(exam.courseSlug), requiredQuizzes: getCertificationRequirement(exam.courseSlug)?.requiredQuizSlugs ?? [] })), ...studioCourses.flatMap((course) => course.finalExam ? [{ exam: course.finalExam.definition, questionPool: course.finalExam.questions, course: studioCourseToAcademyCourse(course), requiredQuizzes: course.quizzes.filter((quiz) => quiz.allowProgressWithoutPassing === false).map((quiz) => quiz.slug) }] : [])];

  return <div className="space-y-8">
    <div className="rounded-[28px] border border-green-100 bg-white p-7">
      <div className="flex items-center gap-3"><ShieldCheck className="text-[#0B5D3B]" /><h2 className="text-2xl font-extrabold text-[#063D2E]">{text("Conditions dâ€™Ã©ligibilitÃ©", "Eligibility requirements")}</h2></div>
      <div className="mt-5 grid gap-3 md:grid-cols-3">{[text("Cours et cas pratiques terminÃ©s", "Courses and case studies completed"), text("Tous les quiz obligatoires rÃ©ussis", "All required quizzes passed"), text(`CrÃ©neau rÃ©servÃ© au moins ${data.policy.minimumLeadDays} jours avant`, `Slot booked at least ${data.policy.minimumLeadDays} days in advance`)].map((label) => <div key={label} className="flex items-center gap-3 rounded-2xl bg-[#F8FAFC] p-4 text-sm font-semibold text-slate-700"><CheckCircle2 size={18} className="shrink-0 text-[#0B5D3B]" />{label}</div>)}</div>
    </div>
    ...
    {exams.map(({ exam, questionPool, course, requiredQuizzes }) => {
      ...
      return <section key={exam.slug} className="rounded-[28px] border border-green-100 bg-white p-7">
        <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="font-bold uppercase tracking-[0.15em] text-[#F58220]">{exam.code}</p><h2 className="mt-2 text-2xl font-extrabold text-[#063D2E]">{exam.title}</h2></div><span className={`rounded-full px-4 py-2 text-xs font-bold ${eligible ? "bg-green-100 text-green-800" : "bg-slate-100 text-slate-600"}`}>{eligible ? text("Ã‰ligible", "Eligible") : text("Parcours incomplet", "Learning path incomplete")}</span></div>
        <div className="mt-5 grid gap-3 sm:grid-cols-3"><Metric ok={summary.progressPercent >= 100} label={`${text("Progression", "Progress")} ${summary.progressPercent}%`} /><Metric ok={missingQuizzes.length === 0} label={missingQuizzes.length === 0 ? text("Quiz rÃ©ussis", "Quizzes passed") : `${missingQuizzes.length} ${text("quiz manquant(s)", "missing quiz(zes)")}`} /><Metric ok={casesComplete} label={casesComplete ? text("Cas pratiques validÃ©s", "Case studies completed") : text("Cas pratiques Ã  terminer", "Case studies to complete")} /></div>
        ...
        {bookingPolicy && bookingPolicy.status !== "first_attempt" && (
          <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm font-semibold text-amber-950">
            {bookingPolicy.status === "active_booking"
              ? text("Une rÃ©servation est dÃ©jÃ  active pour cette tentative.", "A booking is already active for this attempt.")
              : bookingPolicy.status === "awaiting_conduct_review"
                ? text("RÃ©sultat rÃ©ussi : dÃ©cision finale du surveillant en attente.", "Passed result: awaiting the proctorâ€™s final decision.")
                : bookingPolicy.status === "certified"
                  ? text("Examen et conditions de surveillance validÃ©s.", "Exam and proctoring conditions approved.")
                  : bookingPolicy.status === "attempts_exhausted"
                    ? text("Les trois tentatives autorisÃ©es sont Ã©puisÃ©es.", "All authorized attempts have been used.")
                    : text(
                        `Tentative ${bookingPolicy.attemptNumber}/${exam.maxAttempts <= 0 ? "âˆž" : exam.maxAttempts} : choisissez un crÃ©neau Ã  partir du ${earliestSlot?.toLocaleDateString(locale === "fr" ? "fr-FR" : "en-US")}.`,
                        `Attempt ${bookingPolicy.attemptNumber}/${exam.maxAttempts <= 0 ? "âˆž" : exam.maxAttempts}: choose a slot on or after ${earliestSlot?.toLocaleDateString("en-US")}.`,
                      )}
          </div>
        )}
        ...
        {!poolReady && (
          <div className="mt-5 rounded-2xl border border-red-200 bg-red-50 p-4 text-sm font-semibold text-red-800">
            {text(
              `Banque incomplÃ¨te : ${poolCapacity.qcm}/50 QCM, ${poolCapacity.qcu}/35 QCU et ${poolCapacity.case_study}/15 cas pratiques. RÃ©servation suspendue jusquâ€™au chargement par le Studio.`,
        ...
        {ownBookings.length > 0 && <div className="mt-5 space-y-2">{ownBookings.map((booking) => { const slot = data.slots.find((item) => item.id === booking.slotId); return <div key={booking.id} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-green-100 bg-green-50 p-4"><div><p className="font-bold text-[#063D2E]">{text("Demande", "Request")} : {booking.status}</p><p className="text-sm text-slate-600">{slot ? new Date(slot.startsAt).toLocaleString(locale === "fr" ? "fr-FR" : "en-US") : text("CrÃ©neau supprimÃ©", "Slot deleted")}</p></div>{booking.status === "approved" && <Link href={`/dashboard/exams/proctoring/${booking.id}`} className="rounded-full bg-[#0B5D3B] px-5 py-2.5 text-sm font-bold text-white">{text("AccÃ©der Ã  la salle", "Enter room")}</Link>}</div>; })}</div>}

        <div className="mt-6">
          <h3 className="font-extrabold text-[#063D2E]">{text("CrÃ©neaux disponibles", "Available slots")}</h3>
          {slots.length === 0 ? (
            <p className="mt-3 rounded-2xl bg-slate-50 p-4 text-sm text-slate-600">{text("Aucun crÃ©neau nâ€™a encore Ã©tÃ© publiÃ© par lâ€™administration.", "No slot has been published by the administration yet.")}</p>
          ) : (
              ...
              {slots.map((slot) => {
                ...
                return (
                  <div key={slot.id} className="rounded-2xl border border-slate-200 p-4">
                    ...
                    <p className="mt-2 flex items-center gap-2 text-sm text-slate-500"><Clock3 size={15} />{slot.durationMinutes} min Â· {slot.capacity - occupied} {text("place(s)", "seat(s)")}</p>
                    {slotTooEarly && <p className="mt-2 text-xs font-bold text-amber-700">{text("CrÃ©neau trop proche pour cette reprise.", "This slot is too early for this retake.")}</p>}
                    <button
                      type="button"
                      disabled={!canRequest}
                      onClick={() => {
                        ...
                        try {
                          ...
                          window.alert(result.success ? text("Demande transmise et Ã©preuve individualisÃ©e scellÃ©e.", "Request sent and individualized exam paper sealed.") : result.error);
                        } catch (error) {
                          const message = error instanceof Error && error.message.startsWith("QUESTION_POOL_INSUFFICIENT")
                            ...
                            : text("Impossible de gÃ©nÃ©rer une nouvelle Ã©preuve distincte.", "Unable to generate a distinct new exam paper.");
                    ...
                    >
                      {!eligible ? text("Terminer le parcours", "Complete the learning path") : text(`RÃ©server la tentative ${bookingPolicy?.attemptNumber ?? 1}/${exam.maxAttempts <= 0 ? "âˆž" : exam.maxAttempts}`, `Book attempt ${bookingPolicy?.attemptNumber ?? 1}/${exam.maxAttempts <= 0 ? "âˆž" : exam.maxAttempts}`)}
                    </button>
                  </div>
                );
              })}
