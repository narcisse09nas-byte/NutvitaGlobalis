...
export async function verifyAndFinalizeFlutterwaveOrder(transactionId: string, expectedReference?: string) {
  ...
  const { data: order, error: orderError } = await admin.from("academy_orders").select("id, transaction_reference, total, currency, status").eq("transaction_reference", reference).single();
