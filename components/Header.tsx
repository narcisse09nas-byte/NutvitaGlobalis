...
export default function Header() {
  ...
  return <header className="sticky top-0 z-50 border-b border-forest/10 bg-cream/95 backdrop-blur">
    <div className="container-site flex h-20 min-w-0 max-w-[1800px] items-center justify-between gap-3 xl:gap-6">
      <Link href={localizedPath(locale, "/")} className="flex shrink-0 items-center gap-2">
        ...
        <span className="hidden whitespace-nowrap text-base font-black text-forest min-[1180px]:inline xl:text-lg">NutVita<span className="text-orange">Globalis</span></span>
      </Link>
      <nav className="hidden min-w-0 flex-1 items-center justify-center gap-[clamp(.65rem,1.35vw,1.5rem)] min-[1024px]:flex">{links.slice(0,5).map(([href, label]) => <Link key={href} href={localizedPath(locale, href)} className={`whitespace-nowrap text-[13px] font-semibold transition hover:text-leaf xl:text-sm ${canonical === href.split("#")[0] ? "text-leaf" : "text-slate-600"}`}>{label}</Link>)}
        <div ref={opportunitiesRef} className="relative">
          <button type="button" aria-expanded={opportunitiesOpen} onClick={() => setOpportunitiesOpen(value => !value)} className={`inline-flex items-center gap-1 whitespace-nowrap text-[13px] font-semibold transition hover:text-leaf xl:text-sm ${opportunityActive ? "text-leaf" : "text-slate-600"}`}>{locale === "en" ? "Opportunities" : "Opportunites"}<ChevronDownIcon className={`h-4 transition ${opportunitiesOpen ? "rotate-180" : ""}`}/></button>
        ...
        {links.slice(5).map(([href, label]) => <Link key={href} href={localizedPath(locale, href)} className={`whitespace-nowrap text-[13px] font-semibold transition hover:text-leaf xl:text-sm ${canonical === href ? "text-leaf" : "text-slate-600"}`}>{label}</Link>)}
      </nav>
      <div className="hidden shrink-0 items-center gap-2 min-[1024px]:flex xl:gap-3">
        <LanguageSwitcher compact />
        {accountControls}
      </div>
      <button type="button" aria-label={open ? (locale === "en" ? "Close menu" : "Fermer le menu") : (locale === "en" ? "Open menu" : "Ouvrir le menu")} aria-expanded={open} className="grid h-11 w-11 shrink-0 place-items-center rounded-full border border-forest/15 bg-white text-forest shadow-sm min-[1024px]:hidden" onClick={() => setOpen(!open)}>{open ? <XMarkIcon className="h-6" /> : <Bars3Icon className="h-6" />}</button>
    </div>
    {open && <nav className="container-site max-h-[calc(100dvh-5rem)] grid gap-1 overflow-y-auto border-t py-4 min-[1024px]:hidden">
