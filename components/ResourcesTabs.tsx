...
export default function ResourcesTabs({ articles, premiumResources }: { articles: Article[]; premiumResources?: Array<Record<string, unknown>> | null }) {
  ...
  const premium = premiumResources?.length
    ? premiumResources.map(item => ({ title: String(item.title), description: String(item.description || ""), url: "/connexion?redirect=/espace-client/ressources-premium" }))
    : fallbackPremium[locale].map(title => ({ title, description: english ? "A complete, ready-to-use resource updated regularly." : "Un outil complet, pret a utiliser et regulierement actualise.", url: "/connexion?redirect=/espace-client/ressources-premium" }));
  return <>
