...
export default function DietitianManager({ initial }: { initial: Profile[] }) {
  const [rows, setRows] = useState(initial), [editing, setEditing] = useState<Profile | null>(null), [message, setMessage] = useState(""), [busy, setBusy] = useState(false);
  async function toggleFreeOnsiteCreation(row: Profile) {
    const enabled = !row.free_onsite_creation;
    setBusy(true);
    const response = await fetch("/api/admin/dietitians", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "toggle_free_onsite_creation", id: row.id, enabled }) });
    setBusy(false);
    const result = await response.json();
    if (!response.ok) { setMessage(result.message || "Erreur"); return; }
    setRows(current => current.map(item => item.id === row.id ? { ...item, free_onsite_creation: enabled } : item));
  }
  async function save() {
    if (!editing) return;
    const updates = {
