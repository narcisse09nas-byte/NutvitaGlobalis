...
export default function PaymentManager({initial,accounts:initialAccounts}:{initial:Row[];accounts:Row[]}){
  ...
  function statusLabel(row:Row){
    if(row.provider==="manual"&&row.status==="pending")return row.proof_submitted_at?"Preuve soumise":"En attente de preuve";
    if(row.status==="succeeded")return "Valide";
    if(row.status==="failed")return "Refuse";
    if(row.status==="cancelled")return "Annule";
    return row.status;
  }
