...
export default function WrittenTest({ eligible, completed, candidateId, candidateName }: { eligible: boolean; completed: boolean; candidateId: string; candidateName: string }) {
  const [test, setTest] = useState<TestData | null>(null), [seconds, setSeconds] = useState(0), [message, setMessage] = useState(""), [loading, setLoading] = useState(false), [submitting, setSubmitting] = useState(false), [identity, setIdentity] = useState(false), [photo, setPhoto] = useState<Blob | null>(null), [camera, setCamera] = useState(false), [cameraSkipped, setCameraSkipped] = useState(false), [switches, setSwitches] = useState(0), [activeQuestion, setActiveQuestion] = useState<Question | null>(null);
  const saveTimer = useRef<ReturnType<typeof setTimeout> | null>(null), submittingRef = useRef(false), video = useRef<HTMLVideoElement>(null), stream = useRef<MediaStream | null>(null), router = useRouter();
  ...
  async function submit(expired = false) {
    ...
    if (!expired && !confirm("Terminer et soumettre definitivement le test ?")) return;
