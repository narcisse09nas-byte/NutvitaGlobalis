...
export default function AccountAuth({ mode, redirectTo = "/", initialIdentifier = "",locale="fr", serviceLabel }: { mode: "login" | "signup"; redirectTo?: string; initialIdentifier?: string;locale?:"fr"|"en";serviceLabel?:string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = { email, password, full_name: String(fd.get("full_name")), whatsapp_phone: String(fd.get("whatsapp_phone")), country: String(fd.get("country")), country_code: String(fd.get("country_code")), state_region: String(fd.get("state_region")), city: String(fd.get("city")) === "__other" ? String(fd.get("other_city")) : String(fd.get("city")), other_city: String(fd.get("other_city") || ""), promo_code: String(fd.get("promo_code") || "").trim(), accepted_terms: fd.get("terms") === "on", accepted_privacy: fd.get("privacy") === "on", redirect_to: redirectTo };
    try {
  ...
  return <div className={`w-full ${mode === "signup" ? "max-w-2xl" : "max-w-md"} rounded-3xl bg-white p-8 shadow-2xl`}><Link href="/" className="text-xl font-black text-forest">NutVita<span className="text-orange">Globalis</span></Link><h1 className="mt-7 text-3xl font-black">{mode === "login" ? serviceLabel ? tx("Connexion au service","Service sign-in") : tx("Connexion au portail","Portal sign-in") : tx("Creer mon compte","Create my account")}</h1>{serviceLabel&&mode==="login"&&<p className="mt-2 font-bold text-leaf">{serviceLabel}</p>}<p className="mt-2 text-slate-500">{mode === "login" ? tx("Utilisez votre email ou votre nom d utilisateur. Après connexion, vous serez dirigé vers le service demandé.","Use your email address or username. After signing in, you will be directed to the requested service.") : tx("Un compte est requis avant tout achat.","An account is required before any purchase.")}</p>{mode==='login'&&hasLocalAdminMode()&&<p className="mt-4 rounded-xl bg-mint p-3 text-sm"><b>{tx("Compte de demonstration","Demo account")}</b><br/>client.demo {tx("ou","or")} client@nutvitaglobalis.com</p>}<form onSubmit={submit} className="mt-7 grid gap-4">{mode === "signup" ? <SignupFields locale={locale}/> : <><label className="grid gap-2 text-sm font-bold">{tx("Email ou nom d utilisateur","Email or username")}<input name="email" type="text" defaultValue={initialIdentifier} required className="admin-input"/></label><label className="grid gap-2 text-sm font-bold">{tx("Mot de passe","Password")}<input name="password" type="password" minLength={8} required className="admin-input"/></label></>}{message && <p className="rounded-xl bg-mint p-3 text-sm">{message}</p>}<button disabled={loading} className="btn-primary">{loading ? tx("Patientez...","Please wait...") : mode === "login" ? tx("Se connecter","Sign in") : tx("Creer mon compte","Create my account")}</button></form><div className="mt-5 flex flex-wrap justify-between gap-3 text-sm font-bold text-leaf">{mode === "login" ? <><Link href={`/inscription?redirect=${encodeURIComponent(redirectTo)}`}>{tx("Creer un compte","Create an account")}</Link><Link href="/mot-de-passe-oublie">{tx("Mot de passe oublie","Forgot password")}</Link></> : <Link href={`/connexion?redirect=${encodeURIComponent(redirectTo)}`}>{tx("J ai deja un compte","I already have an account")}</Link>}</div></div>;
}
