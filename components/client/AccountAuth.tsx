...
export default function AccountAuth({ mode, redirectTo = "/espace-client", initialIdentifier = "" }: { mode: "login" | "signup"; redirectTo?: string; initialIdentifier?: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const profile = { full_name: String(fd.get("full_name")), whatsapp_phone: String(fd.get("whatsapp_phone")), country: String(fd.get("country")), country_code: String(fd.get("country_code")), state_region: String(fd.get("state_region")), city: String(fd.get("city")) === "__other" ? String(fd.get("other_city")) : String(fd.get("city")), other_city: String(fd.get("other_city") || ""), account_type: "client", accepted_terms_at: new Date().toISOString(), accepted_privacy_at: new Date().toISOString() };
    const callback = `${location.origin}/auth/callback?next=${encodeURIComponent(redirectTo)}`;
    const result = await supabase.auth.signUp({ email, password, options: { data: profile, emailRedirectTo: callback } });
    if (result.error) { setMessage(result.error.message); setLoading(false); return; }
    if (result.data.user && result.data.session) await supabase.from("client_profiles").upsert({ id: result.data.user.id, email, ...profile });
    if (result.data.session) { router.push(redirectTo); router.refresh(); return; }
    setMessage("Compte cree. Confirmez votre adresse email avant de vous connecter."); setLoading(false);
  }
