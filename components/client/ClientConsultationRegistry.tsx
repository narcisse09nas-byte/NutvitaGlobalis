...
export default function ClientConsultationRegistry({ consultations, userEmail }: { consultations: Row[]; userEmail:string }) {
  ...
  return <section className="rounded-2xl border bg-white p-6">
    ...
    <div className="mt-5 overflow-x-auto rounded-xl border"><table className="w-full min-w-[720px] text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="p-4">Date</th><th className="p-4">Consultation</th><th className="p-4">Objectif principal</th><th className="p-4">Prochain rendez-vous</th><th className="p-4">Statut</th><th className="p-4 text-right">Actions</th></tr></thead><tbody className="divide-y">{consultations.map(row => <tr key={row.id}><td className="p-4">{new Date(row.finalized_at || row.scheduled_at).toLocaleDateString("fr-FR")}</td><td className="p-4 font-bold text-forest">{row.reason || "Consultation nutritionnelle"}</td><td className="p-4">{array(row.goals)[0]?.label || "A definir"}</td><td className="p-4">{row.next_appointment_at ? new Date(row.next_appointment_at).toLocaleString("fr-FR") : "Non programme"}</td><td className="p-4"><span className="rounded-full bg-mint px-3 py-1 text-xs font-bold text-leaf">Finalisee</span></td><td className="p-4 text-right"><button type="button" onClick={() => setSelected(row)} className="btn-secondary px-4 py-2"><EyeIcon className="mr-2 h-4"/>Voir</button></td></tr>)}{!consultations.length && <tr><td colSpan={6} className="p-10 text-center text-slate-400">Aucune consultation finalisee.</td></tr>}</tbody></table></div>
