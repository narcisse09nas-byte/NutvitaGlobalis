...
export default function HealthTrendsDashboard({anthropometry,biology,food,wellness=[],locale="fr"}:{anthropometry:Row[];biology:Row[];food:Row[];wellness?:Row[];locale?:"fr"|"en"}) {
  ...
  if(!available.length)return <Empty locale={locale}/>;
  return <div className="grid gap-6">
    ...
    {!!cardio.length&&<Chart title="Tension artérielle et pouls" metrics={cardio} sources={sources} range={range} locale={locale}/>}
    <div className="grid gap-6 xl:grid-cols-2">{others.map(m=><Chart key={m.key} title={m.label} metrics={[m]} sources={sources} range={range} locale={locale}/>)}</div>
  </div>;
}

function Chart({title,metrics,sources,range,locale}:{title:string;metrics:Metric[];sources:Record<Source,Row[]>;range:{min:number;max:number};locale:"fr"|"en"}){
...
function Empty({locale}:{locale:"fr"|"en"}){return <div className="rounded-3xl border bg-white p-12 text-center"><h2 className="text-xl font-black text-forest">Aucune tendance disponible</h2><p className="mt-2 text-slate-500">Enregistrez d’abord vos paramètres de santé.</p></div>}
