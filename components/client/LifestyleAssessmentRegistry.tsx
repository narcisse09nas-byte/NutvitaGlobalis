...
import { assessmentQuestions, buildAssessmentSnapshot, questionReference, scoreToLegacyLevel, type AssessmentKind } from "@/lib/wellness-assessments-v2";
...
export default function LifestyleAssessmentRegistry({clientId,initialRows,locale}:{clientId:string;initialRows:Row[];locale:"fr"|"en"}){
  ...
  return <div className="grid gap-6">
    ...
    <div className="grid gap-4">{rows.map(row=><article key={row.id} className="rounded-2xl border bg-white p-5">
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-3">{(["nutrition","activity","lifestyle"] as AssessmentKind[]).map(domain=><section key={domain} className="rounded-2xl bg-slate-50 p-4"><h4 className="font-black text-forest">{domain==="nutrition"?"Nutrition":domain==="activity"?tx("Activité","Activity"):tx("Mode de vie","Lifestyle")}</h4><div className="mt-3 grid gap-2">{(row.questionnaire_snapshot?.[domain]||assessmentQuestions[domain].map((q,index)=>({id:q.id,number:index+1,reference:questionReference(domain,index),question:q.title,answer:null,score:null}))).map((item:Row,index:number)=>{const number=item.number??index+1,question=item.question??assessmentQuestions[domain][index]?.title,answer=item.answer??tx("Réponse non enregistrée","Answer not recorded"),reference=item.reference??questionReference(domain,index);return <div key={`${item.id||index}`} title={`${reference} — ${question}: ${answer}`} className="flex items-center justify-between gap-3 border-b border-slate-200 py-1.5 text-sm"><span className="font-black text-slate-600">Q{number}</span><b className="text-forest">{item.score!=null?`${item.score}/4`:"—"}</b><span className="sr-only">{reference} — {question}: {answer}</span></div>})}</div></section>)}</div>
