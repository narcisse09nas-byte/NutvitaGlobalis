...
export default function ServiceCatalog({ plans, packs, courses, children, taxRate }: { plans: Row[]; packs: Row[]; courses: Row[]; children: Row[]; taxRate: number }) {
  const [filter, setFilter] = useState("all");
  const [selectedChild, setSelectedChild] = useState(children[0]?.id || "");
  const services = useMemo(() => [
    ...plans.map(plan => ({ id: plan.id, kind: "subscription", group: plan.service_type, title: plan.name, price: Number(plan.price_excluding_tax ?? plan.amount), features: plan.features || [], premium: plan.tier === "premium" })),
  ...
  function checkout(item: any) {
    if (item.group === "child_growth" && !selectedChild) {
      alert("Veuillez d'abord ajouter ou selectionner un enfant pour ce service.");
      return;
    }
    const childParam = item.group === "child_growth" ? `&child_id=${encodeURIComponent(selectedChild)}` : "";
  ...
  return <div className="grid gap-6">
    ...
    <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{rows.map(item => { const tax = Math.round(item.price * taxRate) / 100, total = item.price + tax; return <article key={`${item.kind}-${item.id}`} className={`rounded-2xl border bg-white p-6 ${item.premium ? "border-orange" : ""}`}>
      ...
      {item.group === "child_growth" && <div className="mb-4">
        ...
        </label> : <p className="rounded-xl bg-amber-50 p-3 text-sm text-amber-900">Ajoutez d'abord l'enfant dans l'espace croissance avant d'acheter ce suivi.</p>}
      </div>}
