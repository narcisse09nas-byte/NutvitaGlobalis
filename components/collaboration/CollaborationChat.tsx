...
export default function CollaborationChat({ conversations: initial, currentUserId, contacts = [] }: { conversations: Row[]; currentUserId: string; contacts?: Row[] }) {
  const [conversations, setConversations] = useState(initial), [selected, setSelected] = useState(initial[0]?.id || ""), [messages, setMessages] = useState<Row[]>([]), [body, setBody] = useState(""), [error, setError] = useState(""), [conversationQuery,setConversationQuery]=useState("");
  const visibleConversations=useMemo(()=>conversations.filter(c=>(c.title||"Conversation").toLowerCase().includes(conversationQuery.toLowerCase())),[conversations,conversationQuery]);
  async function load() {
    if (!selected) return;
  ...
  return <div className="grid gap-5 lg:grid-cols-[310px_1fr]"><aside className="rounded-2xl border bg-white p-4">{contacts.length > 0 && <form onSubmit={createConversation} className="mb-5 grid gap-2"><input name="title" className="admin-input text-sm" placeholder="Titre du groupe (facultatif)" /><select name="recipient_ids" required multiple className="admin-input min-h-36 text-sm">{contacts.filter(c => c.id !== currentUserId).map(c => <option key={c.id} value={c.id}>{c.full_name} - {c.member_role || c.department || "contact"}</option>)}</select><button className="btn-secondary px-4 py-2">Créer une discussion</button></form>}<h2 className="mb-3 font-black">Conversations</h2><input value={conversationQuery} onChange={e=>setConversationQuery(e.target.value)} className="admin-input mb-3 text-sm" placeholder="Rechercher une conversation..."/><select value={selected} onChange={e=>setSelected(e.target.value)} className="admin-input text-sm"><option value="">Sélectionner une conversation</option>{visibleConversations.map(c=><option key={c.id} value={c.id}>{c.title||"Conversation"}</option>)}</select>{!visibleConversations.length&&<p className="mt-3 text-sm text-slate-400">Aucune conversation correspondante.</p>}</aside><section className="rounded-2xl border bg-white"><div className="grid min-h-[420px] max-h-[520px] content-start gap-3 overflow-y-auto p-5">{messages.map(m => <div key={m.id} className={`max-w-[85%] rounded-2xl p-4 ${m.sender_id === currentUserId ? "ml-auto bg-forest text-white" : "bg-slate-100"}`}><p className="whitespace-pre-wrap text-sm">{m.body}</p><p className={`mt-2 text-[10px] ${m.sender_id === currentUserId ? "text-white/50" : "text-slate-400"}`}>{new Date(m.created_at).toLocaleString("fr-FR")}</p></div>)}</div><form onSubmit={send} className="flex gap-3 border-t p-4"><input value={body} onChange={e => setBody(e.target.value)} className="admin-input" placeholder="Écrire un message..." /><button className="btn-primary px-5"><PaperAirplaneIcon className="h-5" /></button></form>{error && <p className="px-4 pb-4 text-sm text-red-600">{error}</p>}</section></div>;
}
