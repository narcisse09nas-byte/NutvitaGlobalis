...
import type { Achievement, AchievementStatus, Activity, Indicator } from "@/lib/ppm/types";
...
export default function AchievementReviewInbox({ projectId, initial, activities, indicators }: {
  projectId: string; initial: Achievement[]; activities: Activity[]; indicators: Indicator[];
}) {
  ...
  async function submitDecision(event: FormEvent<HTMLFormElement>) {
    ...
    if (deciding.nextStatus === "returned" && !reviewNote) { setSaving(false); setMessage("Un commentaire est obligatoire pour retourner une realisation."); return; }
    ...
    await supabase.from("ppm_history").insert({
      ...
      action: `Realisation ${statusLabels[updated.status].toLowerCase()} — ${updated.title}`, from_status: deciding.row.status, to_status: updated.status, note: reviewNote || undefined,
    });
    if (updated.created_by_email) {
      await supabase.from("ppm_notifications").insert({
        ...
        title: `${statusLabels[updated.status]} — ${updated.title}`, message: reviewNote || undefined,
  ...
  return <div className="grid gap-4">
    <div><h2 className="text-xl font-black text-forest">Validation des realisations</h2><p className="text-sm text-slate-500">{rows.length} realisation(s) en attente de decision</p></div>
    <div className="grid gap-3">
      {rows.map(row => { const activity = activityById.get(row.activity_id); return <article key={row.id} className="rounded-2xl border bg-white p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div><b className="text-forest">{row.title}</b><p className="mt-1 text-xs text-slate-400">{activity?.title}{row.period_label ? ` · ${row.period_label}` : ""} · Progression proposee : {row.progress_percent ?? "—"}%</p></div>
        ...
        {row.indicator_id && row.indicator_contribution != null && <p className="mt-2 text-sm text-slate-600">Contribution indicateur proposee : +{row.indicator_contribution} ({indicatorById.get(row.indicator_id)?.name})</p>}
        <div className="mt-3 flex flex-wrap gap-2">
          {row.status === "submitted" && <button onClick={() => markUnderReview(row)} className="btn-secondary px-3 py-1.5 text-xs">Mettre en revue</button>}
          <button onClick={() => startDecision(row, "validated")} className="btn-primary px-3 py-1.5 text-xs">Valider</button>
          <button onClick={() => startDecision(row, "returned")} className="btn-secondary px-3 py-1.5 text-xs">Retourner</button>
          <button onClick={() => startDecision(row, "rejected")} className="btn-secondary px-3 py-1.5 text-xs">Rejeter</button>
        </div>
      </article>; })}
      {!rows.length && <p className="rounded-2xl border bg-white p-8 text-center text-slate-400">Aucune realisation en attente.</p>}
    </div>
    ...
    {deciding && <div className="fixed inset-0 z-[150] overflow-y-auto bg-slate-950/60 p-4">
      <form onSubmit={submitDecision} className="mx-auto my-10 max-w-lg rounded-[30px] bg-white p-7 shadow-2xl">
        <div className="flex items-start justify-between"><h2 className="text-xl font-black text-forest">{statusLabels[deciding.nextStatus]} — {deciding.row.title}</h2><button type="button" onClick={() => setDeciding(null)} className="text-2xl">×</button></div>
        <div className="mt-5 grid gap-4">
          <label className="grid gap-2 text-sm font-bold">Revu par<input name="reviewed_by_name" className="admin-input" /></label>
          <label className="grid gap-2 text-sm font-bold">Commentaire<textarea name="review_note" rows={3} className="admin-input" /></label>
