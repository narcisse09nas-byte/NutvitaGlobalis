...
export default function ArchiveRegister({ projectId, initial, deliverables, documents }: {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!title) { setSaving(false); setMessage("Le titre est obligatoire."); return; }
  ...
  return <div className="grid gap-4">
    ...
    {(!!suggestedDeliverables.length || !!suggestedDocuments.length) && <div className="rounded-2xl border border-dashed bg-mint/30 p-4">
      <p className="text-sm font-bold text-forest">Suggestions (livrables acceptes / documents finaux pas encore archives)</p>
      <div className="mt-2 grid gap-2">
        {suggestedDeliverables.map(item => <div key={item.id} className="flex items-center justify-between rounded-xl bg-white px-4 py-2 text-sm"><span>Livrable — {item.title}</span><button onClick={() => archiveOne({ title: item.title, source_type: "deliverable", source_id: item.id, file_path: item.file_path })} className="btn-secondary px-3 py-1.5 text-xs">Archiver</button></div>)}
        {suggestedDocuments.map(item => <div key={item.id} className="flex items-center justify-between rounded-xl bg-white px-4 py-2 text-sm"><span>Document — {item.title}</span><button onClick={() => archiveOne({ title: item.title, source_type: "document", source_id: item.id, file_path: item.file_path })} className="btn-secondary px-3 py-1.5 text-xs">Archiver</button></div>)}
      </div>
    </div>}

    <div className="flex items-center justify-between"><p className="text-sm text-slate-500">{rows.length} element(s) archive(s)</p><button type="button" onClick={() => setAdding(true)} className="btn-primary px-4 py-2 text-sm"><PlusIcon className="mr-2 h-4" />Ajouter un autre document</button></div>
    <div className="grid gap-2">
      {rows.map(row => <div key={row.id} className="flex flex-wrap items-center justify-between gap-2 rounded-xl border bg-white px-4 py-3 text-sm">
        ...
        {row.file_path && <button onClick={() => viewFile(row.file_path)} className="text-xs font-bold text-leaf">Voir</button>}
      </div>)}
      {!rows.length && <p className="rounded-2xl border bg-white p-6 text-center text-slate-400">Aucun element archive.</p>}
    </div>
    ...
    {adding && <div className="fixed inset-0 z-[150] overflow-y-auto bg-slate-950/60 p-4">
      <form onSubmit={submit} className="mx-auto my-10 max-w-lg rounded-[30px] bg-white p-7 shadow-2xl">
        <div className="flex items-start justify-between"><h2 className="text-xl font-black text-forest">Ajouter un document a l&apos;archive</h2><button type="button" onClick={() => setAdding(false)} aria-label="Fermer"><XMarkIcon className="h-6" /></button></div>
        <div className="mt-5 grid gap-4">
          ...
          <div className="flex justify-end gap-3"><button type="button" onClick={() => setAdding(false)} className="btn-secondary">Annuler</button><button disabled={saving} className="btn-primary">{saving ? "Enregistrement..." : "Archiver"}</button></div>
        </div>
      </form>
    </div>}
  </div>;
