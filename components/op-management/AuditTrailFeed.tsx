...
export default function AuditTrailFeed({ entries, currentUserId }: { entries: AuditLogEntry[]; currentUserId?: string }) {
  ...
  return <div className="rounded-2xl border bg-white p-6">
    <h2 className="text-lg font-black text-forest">{en ? "Activity log" : "Journal d'activite"}</h2>
    <div className="mt-4 grid gap-3">
      {entries.map(entry => <div key={entry.id} className="flex gap-3 border-l-2 border-mint pl-4">
        <div className="grid gap-0.5">
          ...
          <p className="text-xs text-slate-400">{entry.actor_id === currentUserId ? (en ? "You" : "Vous") : (en ? "A team member" : "Un membre de l'equipe")} · {new Date(entry.created_at).toLocaleString(en ? "en-US" : "fr-FR")}</p>
        </div>
      </div>)}
      {!entries.length && <p className="text-center text-sm text-slate-400">{en ? "No event recorded." : "Aucun evenement enregistre."}</p>}
    </div>
  </div>;
}
