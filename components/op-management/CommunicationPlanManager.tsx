    ...
    <div className="overflow-x-auto rounded-2xl border bg-white">
      <table className="w-full min-w-[980px] text-left text-sm">
        ...
        <tbody>
          ...
          {!rows.length && <tr><td colSpan={7} className="p-10 text-center text-slate-400">{en ? "No communication planned." : "Aucune communication planifiee."}</td></tr>}
        </tbody>
      </table>
    </div>
    ...
    {editing && <div className="fixed inset-0 z-[150] overflow-y-auto bg-slate-950/60 p-4">
      <form onSubmit={submit} className="mx-auto my-10 max-w-2xl rounded-[30px] bg-white p-7 shadow-2xl">
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          ...
          <label className="grid gap-2 text-sm font-bold">{en ? "Stakeholder" : "Partie prenante"}<select name="stakeholder_id" defaultValue={editing !== "new" ? editing.stakeholder_id || "" : ""} className="admin-input"><option value="">{en ? "None" : "Aucune"}</option>{stakeholders.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
          ...
          <label className="grid gap-2 text-sm font-bold">{en ? "Last sent" : "Derniere diffusion"}<input name="last_sent_date" type="date" defaultValue={editing !== "new" ? editing.last_sent_date || "" : ""} className="admin-input" /></label>
