...
const eacMethodLabels: Record<EacMethod, string> = {
  cpi: "BAC / CPI (performance actuelle maintenue)",
  budgeted_rate: "AC + (BAC - EV) (reste au taux budgete)",
  cpi_spi: "AC + (BAC - EV) / (CPI × SPI) (couts et delais continuent d'affecter la performance)",
};
...
function fmt(value: number | null) { return value == null ? "—" : value.toLocaleString("fr-FR", { maximumFractionDigits: 0 }); }
function fmtRatio(value: number | null) { return value == null ? "—" : value.toFixed(2); }
...
export default function EvmDashboard({ projectId, project, settings, workPackages, budgetLines, activities, achievements, expenses, timePhasedRows, risks, issues, initialSnapshots, pmbSnapshots }: {
  ...
  async function saveSnapshot() {
    ...
    setMessage("Instantane enregistre.");
  }
  ...
  async function analyzeWithAi() {
    ...
    setAiLoading(false);
    if (!response.ok) { setMessage(payload.message || "Analyse IA indisponible."); return; }
  ...
  function exportRows(): (string | number)[][] {
    ...
    rows.push(["TOTAL PROJET", project_metrics.bac, project_metrics.pv, project_metrics.ev, project_metrics.ac, project_metrics.sv, project_metrics.cv, project_metrics.spi ?? "", project_metrics.cpi ?? ""]);
    return [header, ...rows];
  }
  ...
  async function exportPdf() {
    ...
    if (!response.ok) { const payload = await response.json().catch(() => ({})); setMessage(payload.message || "Export PDF impossible."); return; }
  ...
  if (gaps.length) return <div className="rounded-2xl border-2 border-dashed border-amber-300 bg-amber-50 p-6">
    <h2 className="text-lg font-black text-amber-900">EVM non disponible</h2>
    <ul className="mt-3 list-disc pl-5 text-sm text-amber-900">{gaps.map(gap => <li key={gap}>{gap}</li>)}</ul>
  </div>;

  return <div className="grid gap-5">
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div><h2 className="text-xl font-black text-forest">Earned Value Performance</h2><p className="text-sm text-slate-500">Status Date : {new Date(statusDate).toLocaleDateString("fr-FR")}</p></div>
      <div className="flex flex-wrap gap-2">
        <button onClick={exportCsv} className="btn-secondary px-4 py-2 text-sm">Exporter CSV</button>
        <button onClick={exportExcel} className="btn-secondary px-4 py-2 text-sm">Exporter Excel</button>
        <button onClick={exportPdf} className="btn-secondary px-4 py-2 text-sm">Exporter PDF</button>
        <button onClick={saveSnapshot} disabled={saving} className="btn-primary px-4 py-2 text-sm">{saving ? "..." : "Enregistrer un instantane"}</button>
      </div>
    </div>

    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      <div className="rounded-2xl border bg-white p-5"><p className="text-xs font-bold uppercase text-slate-400">BAC ({project_metrics.bacSource === "pmb" ? "PMB approuvee" : "budget courant"})</p><b className="mt-2 block text-xl text-forest">{fmt(project_metrics.bac)}</b></div>
      <div className="rounded-2xl border bg-white p-5"><p className="text-xs font-bold uppercase text-slate-400">PV ({project_metrics.pvSource === "time_phased" ? "budget mensualise" : project_metrics.pvSource === "mixed" ? "partiellement estime" : "estimation lineaire"})</p><b className="mt-2 block text-xl text-forest">{fmt(project_metrics.pv)}</b></div>
    ...
    <div className="rounded-2xl border bg-white p-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h3 className="text-sm font-black uppercase text-slate-500">Previsions (EAC)</h3>
        <select value={eacMethod} onChange={event => setEacMethod(event.target.value as EacMethod)} className="admin-input w-auto text-xs">{Object.entries(eacMethodLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select>
      </div>
    ...
    <div className="rounded-2xl border bg-white p-5">
      <h3 className="text-sm font-black uppercase text-slate-500">Courbe en S (PV / EV / AC)</h3>
      <div className="mt-3"><EvmSCurveChart series={monthlySeries} /></div>
    </div>

    <div className="rounded-2xl border bg-white p-5">
      <h3 className="text-sm font-black uppercase text-slate-500">Work Packages necessitant une attention</h3>
      <div className="mt-3 grid gap-3">
        {exceptions.map(({ wp, metrics }) => {
          ...
          return <div key={wp.id} className="rounded-xl border border-red-200 bg-red-50 p-4">
            ...
            <p className="mt-1 text-xs text-slate-500">Responsable : {wp.responsible_name || "—"} · Budget : {fmt(metrics.bac)} · {wpRisks.length} risque(s) · {wpIssues.length} issue(s) ouverte(s)</p>
            <Link href={`/op-management/projets/${projectId}/work-packages/${wp.id}`} className="mt-2 inline-block text-xs font-bold text-leaf">Voir le detail →</Link>
          </div>;
        })}
        {!exceptions.length && <p className="text-sm text-slate-400">Aucun Work Package en dessous des seuils configures.</p>}
      </div>
    </div>

    {snapshots.length > 1 && <div className="grid gap-5 lg:grid-cols-2">
      <div className="rounded-2xl border bg-white p-5">
        <h3 className="text-sm font-black uppercase text-slate-500">Tendance SPI / CPI</h3>
      ...
      <div className="rounded-2xl border bg-white p-5">
        <h3 className="text-sm font-black uppercase text-slate-500">Tendance EAC / VAC</h3>
        <div className="mt-3 h-56"><ResponsiveContainer width="100%" height="100%">
    ...
    <div className="rounded-2xl border bg-white p-5">
      <div className="flex flex-wrap items-center justify-between gap-3"><h3 className="text-sm font-black uppercase text-slate-500">Analyse IA</h3><button onClick={analyzeWithAi} disabled={aiLoading} className="btn-secondary px-4 py-2 text-sm">{aiLoading ? "Analyse..." : "Analyser avec l'IA"}</button></div>
