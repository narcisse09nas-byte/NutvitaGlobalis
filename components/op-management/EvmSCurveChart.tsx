...
export default function EvmSCurveChart({ series, size = "full" }: { series: { month: string; pv: number; ev: number; ac: number }[]; size?: "full" | "mini" }) {
  ...
  if (!series.length) return <p className="text-center text-sm text-slate-400">{en ? "No data yet for the S-curve." : "Pas encore de donnees pour la courbe en S."}</p>;
  return <div style={{ height: size === "full" ? 320 : 160 }}>
    <ResponsiveContainer width="100%" height="100%">
      <LineChart data={series}>
        ...
        <Tooltip formatter={(value: number) => value.toLocaleString(en ? "en-US" : "fr-FR")} />
        {size === "full" && <Legend />}
