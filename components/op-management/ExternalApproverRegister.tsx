...
export default function ExternalApproverRegister({ projectId, initial }: { projectId: string; initial: ExternalApprover[] }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!payload.name || !payload.email) { setSaving(false); setMessage(en ? "Name and email are required." : "Le nom et l'email sont obligatoires."); return; }
  ...
  return <div className="grid gap-4">
    <div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-xl font-black text-forest">{en ? "External approvers" : "Approbateurs externes"}</h2><button onClick={() => setEditing("new")} className="btn-primary px-4 py-2 text-sm"><PlusIcon className="mr-2 h-4" />{en ? "New approver" : "Nouvel approbateur"}</button></div>
    <div className="overflow-x-auto rounded-2xl border bg-white">
      <table className="w-full min-w-[700px] text-left text-sm">
        <thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="p-4">{en ? "Name" : "Nom"}</th><th className="p-4">{en ? "Organization" : "Organisation"}</th><th className="p-4">Email</th><th className="p-4">Action</th></tr></thead>
        <tbody>
          ...
          {!rows.length && <tr><td colSpan={4} className="p-10 text-center text-slate-400">{en ? "No external approver registered." : "Aucun approbateur externe enregistre."}</td></tr>}
        </tbody>
      </table>
    </div>

    {editing && <PPMFormModal icon={UserIcon} title={editing === "new" ? (en ? "New external approver" : "Nouvel approbateur externe") : (en ? "Edit approver" : "Modifier l'approbateur")} onClose={() => setEditing(null)}>
      <form onSubmit={submit} className="grid gap-4 sm:grid-cols-2">
        ...
        <label className="grid gap-2 text-sm font-bold">{en ? "Title" : "Fonction"}<input name="role_title" defaultValue={editing !== "new" ? editing.role_title || "" : ""} className="admin-input" /></label>
        <label className="grid gap-2 text-sm font-bold sm:col-span-2">{en ? "Organization" : "Organisation"}<input name="organization" defaultValue={editing !== "new" ? editing.organization || "" : ""} className="admin-input" /></label>
