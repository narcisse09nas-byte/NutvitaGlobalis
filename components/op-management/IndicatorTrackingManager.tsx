...
import type { Evaluation, EvaluationStatus, EvaluationType, Indicator, MealEntry, MealEntryStatus } from "@/lib/ppm/types";

const evaluationTypeLabels: Record<EvaluationType, string> = { baseline: "Baseline", midline: "Midline", endline: "Endline", final: "Finale", ex_post: "Ex-post" };
const evaluationStatusLabels: Record<EvaluationStatus, string> = { planned: "Planifiee", ongoing: "En cours", completed: "Terminee", cancelled: "Annulee" };
const mealStatusLabels: Record<MealEntryStatus, string> = {
  draft: "Brouillon", submitted: "Soumise", data_quality_review: "Revue qualite des donnees",
  validated: "Validee", returned: "Retournee", rejected: "Rejetee",
};
...
export default function IndicatorTrackingManager({ projectId, indicators, initialEntries, initialEvaluations }: {
  projectId: string; indicators: Indicator[]; initialEntries: MealEntry[]; initialEvaluations: Evaluation[];
}) {
  ...
  async function submitEntry(event: FormEvent<HTMLFormElement>) {
    ...
    if (!payload.indicator_id || !payload.period_label) { setSaving(false); setMessage("L'indicateur et la periode sont obligatoires."); return; }
  ...
  async function advanceEntry(row: MealEntry, nextStatus: MealEntryStatus) {
    ...
    if (nextStatus === "returned" || nextStatus === "rejected") {
      reviewNote = prompt("Commentaire (obligatoire) :");
  ...
  return <div className="grid gap-8">
    <div className="grid gap-4">
      ...
      <div className="overflow-x-auto rounded-2xl border bg-white">
        <table className="w-full min-w-[900px] text-left text-sm">
          <thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="p-4">Indicateur</th><th className="p-4">Periode</th><th className="p-4">Cible</th><th className="p-4">Realise</th><th className="p-4">Source</th><th className="p-4">Statut</th><th className="p-4">Action</th></tr></thead>
          <tbody>
            {entries.map(row => <tr key={row.id} className="border-t align-top">
              ...
              <td className="p-4"><div className="flex flex-wrap gap-2">
                ...
                {(row.status === "submitted" || row.status === "data_quality_review") && <>
                  <button onClick={() => advanceEntry(row, "validated")} className="btn-primary px-3 py-2 text-xs">Valider</button>
    ...
    <div className="grid gap-4">
      <div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-xl font-black text-forest">Evaluations</h2><button onClick={() => setEditingEvaluation("new")} className="btn-primary px-4 py-2 text-sm"><PlusIcon className="mr-2 h-4" />Nouvelle evaluation</button></div>
      <div className="grid gap-3">
        {evaluations.map(row => <article key={row.id} className="rounded-2xl border bg-white p-5">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div><b className="text-forest">{row.title}</b><p className="mt-1 text-xs text-slate-400">{evaluationTypeLabels[row.type]}{row.planned_date ? ` · Planifiee le ${new Date(row.planned_date).toLocaleDateString("fr-FR")}` : ""}</p></div>
            <span className="rounded-full bg-mint px-3 py-1 text-xs font-bold text-forest">{evaluationStatusLabels[row.status]}</span>
          </div>
          {row.key_findings && <p className="mt-2 text-sm text-slate-600"><b>Constats :</b> {row.key_findings}</p>}
          <button onClick={() => setEditingEvaluation(row)} className="btn-secondary mt-3 px-3 py-1.5 text-xs">Modifier</button>
        </article>)}
        {!evaluations.length && <p className="rounded-2xl border bg-white p-8 text-center text-slate-400">Aucune evaluation enregistree.</p>}
      </div>
    </div>
    ...
    {editingEntry && <div className="fixed inset-0 z-[150] overflow-y-auto bg-slate-950/60 p-4">
      <form onSubmit={submitEntry} className="mx-auto my-10 max-w-xl rounded-[30px] bg-white p-7 shadow-2xl">
        ...
        <div className="mt-5 grid gap-4">
          ...
          <label className="grid gap-2 text-sm font-bold">Collecte par<input name="collected_by_name" defaultValue={editingEntry !== "new" ? editingEntry.collected_by_name || "" : ""} className="admin-input" /></label>
          <label className="grid gap-2 text-sm font-bold">Date de collecte<input name="collection_date" type="date" defaultValue={editingEntry !== "new" ? editingEntry.collection_date || "" : ""} className="admin-input" /></label>
          <label className="grid gap-2 text-sm font-bold">Commentaires<textarea name="comments" rows={2} defaultValue={editingEntry !== "new" ? editingEntry.comments || "" : ""} className="admin-input" /></label>
          {message && <p className="rounded-xl bg-amber-50 p-3 text-sm font-bold text-amber-900">{message}</p>}
          <div className="flex justify-end gap-3"><button type="button" onClick={() => setEditingEntry(null)} className="btn-secondary">Annuler</button><button disabled={saving} className="btn-primary">{saving ? "Enregistrement..." : "Enregistrer"}</button></div>
        </div>
      </form>
    </div>}
    ...
    {editingEvaluation && <div className="fixed inset-0 z-[150] overflow-y-auto bg-slate-950/60 p-4">
      <form onSubmit={submitEvaluation} className="mx-auto my-10 max-w-2xl rounded-[30px] bg-white p-7 shadow-2xl">
        <div className="flex items-start justify-between"><h2 className="text-2xl font-black text-forest">{editingEvaluation === "new" ? "Nouvelle evaluation" : "Modifier l'evaluation"}</h2><button type="button" onClick={() => setEditingEvaluation(null)} aria-label="Fermer"><XMarkIcon className="h-6" /></button></div>
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          <label className="grid gap-2 text-sm font-bold sm:col-span-2">Titre<input name="title" defaultValue={editingEvaluation !== "new" ? editingEvaluation.title : ""} required className="admin-input" /></label>
          <label className="grid gap-2 text-sm font-bold">Type<select name="type" defaultValue={editingEvaluation !== "new" ? editingEvaluation.type : "midline"} className="admin-input">{Object.entries(evaluationTypeLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
          <label className="grid gap-2 text-sm font-bold">Statut<select name="status" defaultValue={editingEvaluation !== "new" ? editingEvaluation.status : "planned"} className="admin-input">{Object.entries(evaluationStatusLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
          <label className="grid gap-2 text-sm font-bold">Date planifiee<input name="planned_date" type="date" defaultValue={editingEvaluation !== "new" ? editingEvaluation.planned_date || "" : ""} className="admin-input" /></label>
          <label className="grid gap-2 text-sm font-bold">Date reelle<input name="actual_date" type="date" defaultValue={editingEvaluation !== "new" ? editingEvaluation.actual_date || "" : ""} className="admin-input" /></label>
          <label className="grid gap-2 text-sm font-bold sm:col-span-2">Methodologie<textarea name="methodology" rows={2} defaultValue={editingEvaluation !== "new" ? editingEvaluation.methodology || "" : ""} className="admin-input" /></label>
          <label className="grid gap-2 text-sm font-bold sm:col-span-2">Constats cles<textarea name="key_findings" rows={2} defaultValue={editingEvaluation !== "new" ? editingEvaluation.key_findings || "" : ""} className="admin-input" /></label>
          <label className="grid gap-2 text-sm font-bold sm:col-span-2">Recommandations<textarea name="recommendations" rows={2} defaultValue={editingEvaluation !== "new" ? editingEvaluation.recommendations || "" : ""} className="admin-input" /></label>
          {message && <p className="rounded-xl bg-amber-50 p-3 text-sm font-bold text-amber-900 sm:col-span-2">{message}</p>}
          <div className="flex justify-end gap-3 sm:col-span-2"><button type="button" onClick={() => setEditingEvaluation(null)} className="btn-secondary">Annuler</button><button disabled={saving} className="btn-primary">{saving ? "Enregistrement..." : "Enregistrer"}</button></div>
        </div>
      </form>
    </div>}
  </div>;
