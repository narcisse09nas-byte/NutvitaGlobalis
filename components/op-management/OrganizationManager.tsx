...
export default function OrganizationManager({ initial }: { initial: Organization[] }) {
  ...
  return <div className="grid gap-6">
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div><h1 className="text-3xl font-black text-forest">Organisations</h1><p className="mt-1 text-sm text-slate-500">{rows.length} organisation(s) enregistree(s)</p></div>
      <button onClick={() => setEditing("new")} className="btn-primary"><PlusIcon className="mr-2 h-5" />Nouvelle organisation</button>
    </div>

    <div className="overflow-x-auto rounded-2xl border bg-white">
      <table className="w-full min-w-[720px] text-left text-sm">
        <thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="p-4">Nom</th><th className="p-4">Code</th><th className="p-4">Pays</th><th className="p-4">Statut</th><th className="p-4">Action</th></tr></thead>
        <tbody>
          {rows.map(row => <tr key={row.id} className="border-t align-top">
            ...
            <td className="p-4"><div className="flex flex-wrap gap-2"><Link href={`/op-management/organisations/${row.id}`} className="btn-secondary px-3 py-2 text-xs">Ouvrir</Link><button onClick={() => setEditing(row)} className="btn-secondary px-3 py-2 text-xs">Modifier</button></div></td>
          </tr>)}
          {!rows.length && <tr><td colSpan={5} className="p-10 text-center text-slate-400">Aucune organisation. Creez la premiere pour commencer.</td></tr>}
        </tbody>
      </table>
    </div>
    ...
    {editing && <div className="fixed inset-0 z-[150] overflow-y-auto bg-slate-950/60 p-4">
      <form onSubmit={submit} className="mx-auto my-10 max-w-xl rounded-[30px] bg-white p-7 shadow-2xl">
        <div className="flex items-start justify-between"><h2 className="text-2xl font-black text-forest">{editing === "new" ? "Nouvelle organisation" : "Modifier l'organisation"}</h2><button type="button" onClick={() => setEditing(null)} aria-label="Fermer"><XMarkIcon className="h-6" /></button></div>
        <div className="mt-6 grid gap-4">
          <label className="grid gap-2 text-sm font-bold">Nom<input name="name" defaultValue={editing !== "new" ? editing.name : ""} required className="admin-input" /></label>
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <label className="grid gap-2 text-sm font-bold">Pays<input name="country" defaultValue={editing !== "new" ? editing.country || "" : ""} className="admin-input" /></label>
          </div>
