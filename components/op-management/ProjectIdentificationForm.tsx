...
import type { Portfolio, PPMResource, Program, Project, ProjectPriority, ProjectType, Stakeholder } from "@/lib/ppm/types";
...
export default function ProjectIdentificationForm({ project, portfolios, programs, staff = [], stakeholders = [] }: { project: Project; portfolios: Portfolio[]; programs: Program[]; staff?: PPMResource[]; stakeholders?: Stakeholder[] }) {
  ...
  return <form onSubmit={submit} className="grid gap-5 rounded-2xl border bg-white p-6">
    <div className="grid gap-4 sm:grid-cols-2">
      ...
      <label className="grid gap-2 text-sm font-bold">{en ? "Project manager" : "Chef de projet"}<SearchableSelect name="project_manager_name" options={staffOptions} defaultValue={project.project_manager_name || ""} allowOther otherLabel={en ? "Project manager name" : "Nom du chef de projet"} placeholder={en ? "Select a staff member..." : "Selectionner un membre du staff..."} /></label>
      <label className="grid gap-2 text-sm font-bold">{en ? "Project manager email" : "Email chef de projet"}<input name="project_manager_email" type="email" defaultValue={project.project_manager_email || ""} className="admin-input" /></label>
      <label className="grid gap-2 text-sm font-bold">Sponsor<SearchableSelect name="sponsor_name" options={sponsorOptions} defaultValue={project.sponsor_name || ""} allowOther otherLabel={en ? "Sponsor name" : "Nom du sponsor"} placeholder={en ? "Select..." : "Selectionner..."} /></label>
      <label className="grid gap-2 text-sm font-bold">{en ? "Sponsor email" : "Email sponsor"}<input name="sponsor_email" type="email" defaultValue={project.sponsor_email || ""} className="admin-input" /></label>
      <label className="grid gap-2 text-sm font-bold">{en ? "Responsible unit" : "Unite responsable"}<input name="responsible_unit" defaultValue={project.responsible_unit || ""} className="admin-input" /></label>
      ...
      <label className="grid gap-2 text-sm font-bold sm:col-span-2">{en ? "Zones / sites (comma-separated)" : "Zones / sites (separes par des virgules)"}<input name="sites" defaultValue={(project.sites || []).join(", ")} className="admin-input" /></label>
      ...
      <label className="grid gap-2 text-sm font-bold">{en ? "Donor" : "Bailleur"}<input name="donor_name" defaultValue={project.donor_name || ""} className="admin-input" /></label>
