...
const typeLabels: Record<RequirementType, { fr: string; en: string }> = {
  ...
  security: { fr: "Securite", en: "Security" }, operational: { fr: "Operationnelle", en: "Operational" }, other: { fr: "Autre (a preciser)", en: "Other (specify)" },
};
...
export default function RequirementsRegister({ projectId, initial, staff = [] }: { projectId: string; initial: Requirement[]; staff?: PPMResource[] }) {
  ...
  function openEditor(row: Requirement | "new") {
    setMessage("");
    setTypeOther(row !== "new" && row.type === "other");
    setEditing(row);
  }
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      type_other_detail: String(form.get("type") || "") === "other" ? (String(form.get("type_other_detail") || "").trim() || null) : null,
      ...
      <table className="w-full min-w-[900px] text-left text-sm">
        ...
        <tbody>
          {rows.map(row => <tr key={row.id} className="border-t align-top">
            ...
            <td className="p-4">{row.type === "other" && row.type_other_detail ? row.type_other_detail : typeLabels[row.type][locale]}</td>
    ...
    {editing && <div className="fixed inset-0 z-[150] overflow-y-auto bg-forest/90 p-4">
      <form onSubmit={submit} className="mx-auto my-10 max-w-2xl rounded-[30px] bg-white p-7 shadow-2xl">
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          ...
          <label className="grid gap-2 text-sm font-bold">Type<select name="type" defaultValue={editing !== "new" ? editing.type : "functional"} onChange={event => setTypeOther(event.target.value === "other")} className="admin-input">{Object.entries(typeLabels).map(([value, label]) => <option key={value} value={value}>{label[locale]}</option>)}</select>
            {typeOther && <input name="type_other_detail" placeholder={en ? "Specify the type" : "Preciser le type"} defaultValue={editing !== "new" ? editing.type_other_detail || "" : ""} className="admin-input mt-2" required />}
          </label>
