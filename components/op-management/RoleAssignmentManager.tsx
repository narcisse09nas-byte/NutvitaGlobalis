...
export default function RoleAssignmentManager({ scopeType, scopeId, scopeLabel }: { scopeType: "organization" | "portfolio" | "program" | "project"; scopeId: string; scopeLabel: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setSaving(false);
    if (!response.ok) { setMessage(payload.message || "Attribution impossible."); return; }
  ...
  async function revoke(id: string) {
    if (!confirm("Retirer ce role ?")) return;
    const response = await fetch("/api/ppm/role-assignments", {
  ...
  return <div className="rounded-2xl border bg-white p-6">
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div><h2 className="text-lg font-black text-forest">Roles &amp; permissions</h2><p className="text-xs text-slate-400">Perimetre : {scopeLabel}</p></div>
      <button onClick={() => setCreating(true)} className="btn-secondary px-3 py-2 text-xs"><PlusIcon className="mr-1 h-4" />Assigner un role</button>
    </div>
    <div className="mt-4 grid gap-2">
      {rows.map(row => <div key={row.id} className="flex flex-wrap items-center justify-between gap-2 rounded-xl bg-slate-50 px-4 py-2.5 text-sm">
        ...
        <button onClick={() => revoke(row.id)} className="text-xs font-bold text-red-600">Retirer</button>
      </div>)}
      {!rows.length && <p className="text-sm text-slate-400">Aucun role assigne a ce perimetre.</p>}
    </div>
    ...
    {creating && <div className="fixed inset-0 z-[150] overflow-y-auto bg-slate-950/60 p-4">
      <form onSubmit={submit} className="mx-auto my-10 max-w-md rounded-[30px] bg-white p-7 shadow-2xl">
        <div className="flex items-start justify-between"><h2 className="text-xl font-black text-forest">Assigner un role</h2><button type="button" onClick={() => setCreating(false)} aria-label="Fermer"><XMarkIcon className="h-6" /></button></div>
        <div className="mt-5 grid gap-4">
          <label className="grid gap-2 text-sm font-bold">Email du compte NutVitaGlobalis<input name="email" type="email" required className="admin-input" /></label>
          <label className="grid gap-2 text-sm font-bold">Role<select name="role" className="admin-input">{PPM_ROLES.map(role => <option key={role} value={role}>{PPM_ROLE_LABELS[role]}</option>)}</select></label>
          {message && <p className="rounded-xl bg-amber-50 p-3 text-sm font-bold text-amber-900">{message}</p>}
          <div className="flex justify-end gap-3"><button type="button" onClick={() => setCreating(false)} className="btn-secondary">Annuler</button><button disabled={saving} className="btn-primary">{saving ? "Enregistrement..." : "Assigner"}</button></div>
        </div>
      </form>
    </div>}
  </div>;
