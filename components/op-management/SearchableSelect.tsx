...
export default function SearchableSelect({ name, options, defaultValue, placeholder, required, allowOther, otherLabel, onChange }: {
  ...
  return <div ref={containerRef} className="relative grid gap-2">
    ...
    <button type="button" onClick={() => setOpen(current => !current)} className="admin-input flex items-center justify-between text-left">
      <span className={selected ? "" : "text-slate-400"}>{selected ? selected.label : placeholder || "Selectionner..."}</span>
    ...
    {open && <div className="absolute z-30 mt-1 w-full rounded-xl border bg-white p-2 shadow-xl">
      <div className="flex items-center gap-2 rounded-lg bg-slate-50 px-2 py-1.5">
        ...
        <input autoFocus value={query} onChange={event => setQuery(event.target.value)} placeholder="Rechercher..." className="w-full bg-transparent text-sm outline-none" />
      </div>
      <div className="mt-2 max-h-56 overflow-y-auto">
        ...
        {!filtered.length && <p className="px-3 py-2 text-sm text-slate-400">Aucun resultat.</p>}
        {allowOther && <button type="button" onClick={() => pick("__other__")} className="block w-full rounded-lg px-3 py-2 text-left text-sm font-bold text-leaf hover:bg-mint">Autre (preciser)</button>}
      </div>
    </div>}
    {isOther && <input name={name} placeholder={otherLabel || "Preciser..."} required={required} className="admin-input" />}
  </div>;
}
