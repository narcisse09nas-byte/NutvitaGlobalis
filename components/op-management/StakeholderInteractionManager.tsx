...
import type { Stakeholder, StakeholderInteraction, StakeholderLevel, StakeholderPosition } from "@/lib/ppm/types";
...
export default function StakeholderInteractionManager({ projectId, initial, stakeholders }: {
  projectId: string; initial: StakeholderInteraction[]; stakeholders: Stakeholder[];
}) {
    ...
    <div className="grid gap-3">
      {rows.map(row => <article key={row.id} className="rounded-2xl border bg-white p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div><b className="text-forest">{stakeholderLabel(row.stakeholder_id)}</b><p className="mt-1 text-xs text-slate-400">{row.interaction_type}{row.interaction_date ? ` · ${new Date(row.interaction_date).toLocaleDateString("fr-FR")}` : ""}</p></div>
          {row.proposed_position && <span className="rounded-full bg-amber-50 px-3 py-1 text-xs font-bold text-amber-800">Position proposee : {positionLabels[row.proposed_position]} ({row.position_change_status})</span>}
        </div>
        {row.engagement_observed && <p className="mt-2 text-sm text-slate-600"><b>Engagement observe :</b> {row.engagement_observed}</p>}
        {row.concerns && <p className="mt-2 text-sm text-slate-600"><b>Preoccupations :</b> {row.concerns}</p>}
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          {row.position_change_status === "proposed" && <>
            <button onClick={() => decidePositionChange(row, true)} className="btn-primary px-3 py-1.5 text-xs">Approuver le changement</button>
            <button onClick={() => decidePositionChange(row, false)} className="btn-secondary px-3 py-1.5 text-xs">Rejeter</button>
          </>}
        </div>
      </article>)}
      {!rows.length && <p className="rounded-2xl border bg-white p-8 text-center text-slate-400">Aucune interaction enregistree.</p>}
    </div>

    {editing && <div className="fixed inset-0 z-[150] overflow-y-auto bg-slate-950/60 p-4">
      <form onSubmit={submit} className="mx-auto my-10 max-w-2xl rounded-[30px] bg-white p-7 shadow-2xl">
        <div className="flex items-start justify-between"><h2 className="text-2xl font-black text-forest">{editing === "new" ? "Enregistrer une interaction" : "Modifier"}</h2><button type="button" onClick={() => setEditing(null)} aria-label="Fermer"><XMarkIcon className="h-6" /></button></div>
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          <label className="grid gap-2 text-sm font-bold sm:col-span-2">Partie prenante<select name="stakeholder_id" defaultValue={editing !== "new" ? editing.stakeholder_id : ""} required className="admin-input"><option value="">Selectionner...</option>{stakeholders.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
          <label className="grid gap-2 text-sm font-bold">Date<input name="interaction_date" type="date" defaultValue={editing !== "new" ? editing.interaction_date || "" : ""} className="admin-input" /></label>
          <label className="grid gap-2 text-sm font-bold">Type d&apos;interaction<input name="interaction_type" defaultValue={editing !== "new" ? editing.interaction_type || "" : ""} className="admin-input" /></label>
          <label className="grid gap-2 text-sm font-bold sm:col-span-2">Participants<input name="participants" defaultValue={editing !== "new" ? editing.participants || "" : ""} className="admin-input" /></label>
          <label className="grid gap-2 text-sm font-bold sm:col-span-2">Objectif<input name="objective" defaultValue={editing !== "new" ? editing.objective || "" : ""} className="admin-input" /></label>
          <label className="grid gap-2 text-sm font-bold sm:col-span-2">Sujets discutes<textarea name="topics_discussed" rows={2} defaultValue={editing !== "new" ? editing.topics_discussed || "" : ""} className="admin-input" /></label>
          <label className="grid gap-2 text-sm font-bold">Preoccupations<textarea name="concerns" rows={2} defaultValue={editing !== "new" ? editing.concerns || "" : ""} className="admin-input" /></label>
          <label className="grid gap-2 text-sm font-bold">Attentes exprimees<textarea name="expectations" rows={2} defaultValue={editing !== "new" ? editing.expectations || "" : ""} className="admin-input" /></label>
          <label className="grid gap-2 text-sm font-bold sm:col-span-2">Engagement observe<textarea name="engagement_observed" rows={2} defaultValue={editing !== "new" ? editing.engagement_observed || "" : ""} className="admin-input" /></label>
          <label className="grid gap-2 text-sm font-bold">Decisions<textarea name="decisions" rows={2} defaultValue={editing !== "new" ? editing.decisions || "" : ""} className="admin-input" /></label>
          <label className="grid gap-2 text-sm font-bold">Actions<textarea name="actions" rows={2} defaultValue={editing !== "new" ? editing.actions || "" : ""} className="admin-input" /></label>
