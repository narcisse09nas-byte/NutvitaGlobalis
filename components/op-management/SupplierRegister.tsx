...
export default function SupplierRegister({ projectId, initial }: { projectId: string; initial: Supplier[] }) {
  ...
  return <div className="grid gap-4">
    <div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-xl font-black text-forest">{en ? "Suppliers" : "Fournisseurs"}</h2><button onClick={() => setEditing("new")} className="btn-primary px-4 py-2 text-sm"><PlusIcon className="mr-2 h-4" />{en ? "New supplier" : "Nouveau fournisseur"}</button></div>
