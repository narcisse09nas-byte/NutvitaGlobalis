...
type Application = {
  ...
  professional_title?: string; highest_degree?: string; years_experience?: number;
  cover_letter?: string; documents?: Record<string, { path: string; name: string }>;
};
...
export default function StaffCandidatePortal({ email, fullName, settings }: { email: string; fullName: string; settings: RecruitmentUiSettings }) {
  ...
  return <div className="min-h-screen bg-slate-100">
    <header className="border-b bg-white"><div className="mx-auto flex max-w-[1500px] flex-wrap items-center justify-between gap-4 px-5 py-5"><div><p className="text-xl font-black text-forest">NutVita<span className="text-orange">Candidat</span></p><p className="text-xs text-slate-500">{english?"Join NutVitaGlobalis":"Rejoignez NutVitaGlobalis"}</p></div><div className="flex items-center gap-4"><div className="flex rounded-full bg-slate-100 p-1"><button onClick={()=>setEnglish(false)} className={`rounded-full px-3 py-1 text-xs font-black ${!english?"bg-forest text-white":""}`}>FR</button><button onClick={()=>setEnglish(true)} className={`rounded-full px-3 py-1 text-xs font-black ${english?"bg-forest text-white":""}`}>EN</button></div><span className="hidden text-sm text-slate-500 sm:block">{email}</span><button onClick={logout} className="inline-flex items-center gap-2 text-sm font-bold text-forest"><LogOut className="h-4 w-4" />{english?"Sign out":"Deconnexion"}</button></div></div></header>
    <div className="mx-auto grid max-w-[1500px] gap-6 px-5 py-7 lg:grid-cols-[240px_minmax(0,1fr)]"><aside className="flex flex-col self-stretch rounded-3xl bg-gradient-to-b from-[#073f32] to-[#00533f] p-4 text-white shadow-xl lg:sticky lg:top-4"><nav className="grid gap-2">{[["/candidatures",english?"All my applications":"Toutes mes candidatures"],["#dashboard",english?"Dashboard":"Tableau de bord"],["#applications",english?"My applications":"Mes candidatures"],["/candidat/test-ecrit",english?"Written tests":"Tests ecrits"],["/candidat/entretien",english?"Interviews":"Entretiens"],["#notifications",english?"Notifications":"Notifications"],["/signatures",english?"Electronic signatures":"Signatures electroniques"]].map(([href,label])=><a key={href} href={href} className="rounded-xl px-4 py-3 text-sm font-black hover:bg-white/10">{label}</a>)}</nav><div className="mt-auto rounded-2xl bg-white/10 p-4 pt-4"><b>{english?settings.help_title_en:settings.help_title}</b><p className="mt-2 text-xs leading-5 text-white/75">{english?settings.help_text_en:settings.help_text}</p><a href="mailto:contact@nutvitaglobalis.com" className="mt-4 block rounded-xl border border-white/30 p-2 text-center text-xs font-black">{english?"Contact support":"Contacter le support"}</a></div></aside><main id="dashboard" className="grid min-w-0 gap-7 [&_section]:rounded-3xl [&_section]:shadow-sm">
      <div><h1 className="text-3xl font-black text-forest">{english?settings.candidate_title_en:settings.candidate_title} - {fullName}</h1><p className="mt-2 text-slate-500">{english?settings.candidate_intro_en:settings.candidate_intro}</p></div>
      {message && <p className="rounded-lg border bg-white p-4 font-semibold text-forest">{message}</p>}
      <div id="tests"><CandidateWrittenTests /></div>
      {interviews.length > 0 && <section id="interviews" className="rounded-lg border bg-white p-6"><h2 className="text-xl font-black">{english?"My interviews":"Mes entretiens"}</h2><div className="mt-4 grid gap-3">{interviews.map(item => <article key={item.id} className="flex flex-col justify-between gap-4 rounded-md border p-4 sm:flex-row sm:items-center"><div><h3 className="font-black">{item.maximus_staff_applications?.maximus_job_offers?.title}</h3><p className="mt-1 text-sm text-slate-500">{new Date(item.scheduled_at).toLocaleString('fr-FR')} · {item.duration_minutes} minutes</p></div>{item.provider === 'physical' ? <span className="rounded-md border px-4 py-2 text-sm font-bold">{item.meeting_url}</span> : <a href={item.meeting_url} target="_blank" rel="noreferrer" className="rounded-md bg-[#24945f] px-4 py-2 text-center text-sm font-bold text-white">{english?"Join interview":"Rejoindre l entretien"}</a>}</article>)}</div></section>}
      {proposals.length > 0 && <section className="rounded-lg border bg-white p-6"><h2 className="text-xl font-black">{english?"Job offers":"Propositions d embauche"}</h2><div className="mt-4 grid gap-4">{proposals.map(item => <article key={item.id} className="rounded-md border p-5"><div className="flex flex-wrap justify-between gap-3"><div><h3 className="text-lg font-black">{item.position_title}</h3><p className="text-sm text-slate-500">{item.contract_type} · Debut propose: {item.proposed_start_date || 'A convenir'}</p></div><span className="rounded-full bg-mint px-3 py-1 text-xs font-bold text-forest">{item.status}</span></div>{item.salary_amount ? <p className="mt-4 font-black">{Number(item.salary_amount).toLocaleString('fr-FR')} {item.salary_currency}</p> : null}<p className="mt-3 whitespace-pre-wrap text-sm leading-6">{item.terms}</p>{item.status === 'sent' && <div className="mt-5 flex flex-wrap justify-end gap-3 border-t pt-4"><button onClick={() => answerProposal(item.id, 'declined')} className="rounded-md border px-4 py-2 font-bold">Decliner</button><a href="/signatures" className="rounded-md bg-[#24945f] px-4 py-2 font-bold text-white">Lire et signer la proposition</a></div>}</article>)}</div></section>}
      <section id="applications" className="rounded-lg border bg-white p-6"><h2 className="text-xl font-black">{english?"My applications":"Mes candidatures"}</h2><div className="mt-5 overflow-x-auto"><table className="w-full min-w-[850px] text-left text-sm"><thead><tr className="border-b text-slate-500"><th className="p-3">{english?"Position":"Poste"}</th><th className="p-3">{english?"Reference":"Reference"}</th><th className="p-3">{english?"Status":"Statut"}</th><th className="p-3">{english?"Written test":"Test ecrit"}</th><th className="p-3">{english?"Interview":"Entretien"}</th><th className="p-3">{english?"Submitted on":"Soumise le"}</th><th className="p-3">{english?"Dossier":"Dossier"}</th></tr></thead><tbody>{applications.length ? applications.map(item => <tr key={item.id} className="border-b"><td className="p-3 font-semibold">{item.maximus_job_offers?.title}</td><td className="p-3 font-mono text-xs">{item.maximus_job_offers?.reference}</td><td className="p-3"><span className="rounded-full bg-mint px-2.5 py-1 text-xs font-bold text-forest">{statuses[item.status] || item.status}</span></td><td className="p-3">{item.written_test_score ?? 'N/A'}</td><td className="p-3">{item.interview_score ?? 'N/A'}</td><td className="p-3">{item.submitted_at ? new Date(item.submitted_at).toLocaleDateString('fr-FR') : 'N/A'}</td><td className="p-3"><button onClick={() => setViewing(item)} className="rounded-md border px-3 py-1.5 text-xs font-bold">{english?"View":"Voir"}</button></td></tr>) : <tr><td colSpan={7} className="h-28 text-center text-slate-500">Vous n avez encore soumis aucune candidature Staff.</td></tr>}</tbody></table></div></section>
      <section className="rounded-lg border bg-white p-6"><h2 className="text-xl font-black">{english?"Open positions":"Offres ouvertes"}</h2><div className="mt-5 grid gap-3">{offers.map(offer => <article key={offer.id} className="flex flex-col justify-between gap-4 border-b py-4 sm:flex-row sm:items-center"><div><p className="text-xs font-bold uppercase text-leaf">{offer.department} · {offer.contract_type}</p><h3 className="mt-1 text-lg font-black">{offer.title}</h3><p className="mt-1 text-sm text-slate-500">{offer.location}</p></div>{appliedIds.has(offer.id) ? <span className="text-sm font-bold text-slate-400">Candidature deja soumise</span> : <button onClick={() => setSelected(offer)} className="inline-flex items-center gap-2 rounded-md bg-[#24945f] px-4 py-2 text-sm font-bold text-white"><BriefcaseBusiness className="h-4 w-4" />{english?"Apply":"Postuler"}</button>}</article>)}</div></section>
      <section id="notifications" className="rounded-lg border bg-white p-6"><h2 className="text-xl font-black">{english?"Notifications":"Notifications"}</h2><div className="mt-5 grid gap-3">{notifications.length ? notifications.map(item => <article key={item.id} className="rounded-md bg-slate-50 p-4"><div className="flex items-start justify-between gap-4"><div><h3 className="font-black text-forest">{item.title}</h3><p className="mt-2 text-sm leading-6 text-slate-600">{item.message}</p></div><time className="shrink-0 text-xs text-slate-400">{new Date(item.created_at).toLocaleDateString('fr-FR')}</time></div></article>) : <p className="text-sm text-slate-500">Aucune notification.</p>}</div></section>
    </main></div>
    {viewing && <div className="fixed inset-0 z-[95] overflow-y-auto bg-slate-950/55 p-4" onMouseDown={() => setViewing(null)}>
      <section className="mx-auto my-6 w-full max-w-2xl rounded-lg bg-white shadow-2xl" onMouseDown={event => event.stopPropagation()}>
        <header className="flex justify-between border-b p-6"><div><p className="text-xs font-bold uppercase text-orange">{viewing.maximus_job_offers?.reference}</p><h2 className="mt-1 text-2xl font-black">{viewing.maximus_job_offers?.title}</h2><p className="mt-2 text-xs text-slate-400">{english?"This information can no longer be edited.":"Ces informations ne sont plus modifiables."}</p></div><button onClick={() => setViewing(null)}><X className="h-5 w-5" /></button></header>
        <div className="grid gap-5 p-6">
          <div className="grid gap-4 sm:grid-cols-2">
            {[[english?"Full name":"Nom complet", viewing.full_name], [english?"Phone":"Telephone", viewing.phone], [english?"Professional title":"Titre professionnel", viewing.professional_title], [english?"Highest degree":"Diplome le plus eleve", viewing.highest_degree], [english?"Years of experience":"Annees d'experience", viewing.years_experience], [english?"Address":"Adresse", viewing.address]].map(([label, value]) => value ? <div key={label as string}><small className="block text-xs uppercase text-slate-400">{label}</small><b>{value}</b></div> : null)}
          </div>
          {viewing.cover_letter && <div><small className="block text-xs uppercase text-slate-400">{english?"Cover letter":"Lettre de motivation"}</small><p className="mt-2 whitespace-pre-wrap text-sm leading-6">{viewing.cover_letter}</p></div>}
          {viewing.documents && Object.keys(viewing.documents).length > 0 && <div><small className="block text-xs uppercase text-slate-400">{english?"Documents":"Documents"}</small><div className="mt-2 grid gap-2 sm:grid-cols-2">{Object.entries(viewing.documents).map(([field, doc]) => <DocumentLink key={field} bucket="maximus-recruitment" path={doc.path} label={doc.name} />)}</div></div>}
        </div>
      </section>
    </div>}
