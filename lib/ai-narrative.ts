...
export async function enrichChildGrowthNarrative<T extends {
  ...
  const result = await generateStructured<Pick<T, 'summary' | 'professionalSummary' | 'practicalAdvice' | 'parentConclusion' | 'professionalConclusion' | 'indicatorInsights' | 'whoCurveAnalysis' | 'growthStory' | 'influencingFactors' | 'developmentAnalysis' | 'actionPlan' | 'limitations'>>(
    ...
    [
      ...
      'Produis un resume executif de 160 mots maximum couvrant situation generale, variation depuis la visite precedente, trajectoire globale, risques et objectifs.',
      'Distingue obligatoirement la trajectoire depuis la premiere visite de la variation entre les deux visites les plus recentes. Toute perte ponderale recente ou cassure doit etre explicite.',
