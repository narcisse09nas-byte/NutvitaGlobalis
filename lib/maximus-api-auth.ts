...
export async function requireMaximusApi(module: string, required: MaximusFunction = "viewer") {
  ...
  if (active.service !== "maximus" || !["staff","admin"].includes(active.role)) return { error: NextResponse.json({ message: "Ouvrez Maximus avec un rôle autorisé." }, { status: 403 }) };
  const allowedModules = access ? modulesForAccess(Array.isArray(access.units) && access.units.length ? access.units : [access.unit], Array.isArray(access.module_access) ? access.module_access : []) : [];
  if (!access || !allowedModules.includes(module)) return { error: NextResponse.json({ message: "Votre unité ne donne pas accès à ce module Maximus." }, { status: 403 }) };
  if (!access.is_assistant_admin && !access.functions?.includes(required)) return { error: NextResponse.json({ message: `Fonction ${required} requise dans votre unité.` }, { status: 403 }) };
