...
export async function requireMaximusAccess(module?: string) {
  if (hasLocalAdminMode() && !hasSupabaseConfig()) {
    ...
    const admin = { ...localAdmin, email: cookieStore.get("nutvita_local_admin_email")?.value || localAdmin.email, full_name: process.env.LOCAL_ADMIN_NAME || localAdmin.full_name, role: "super_admin" };
  ...
  if (!isSuperAdmin) await requireActivePlatformSession("maximus",["staff","admin"]);
  if (!isSuperAdmin && !access) redirect("/contact?objet=acces-maximus");
  ...
  if (module && module !== "finance/internal-transfers" && !isSuperAdmin && !allowedModules?.includes(module)) {
    if (module === "communications/meetings") {
      const { data: invitation } = await supabase.from("maximus_meeting_participants").select("id").eq("user_id", user.id).neq("invitation_status", "revoked").limit(1).maybeSingle();
      if (!invitation) redirect("/maximus?acces=refuse");
    } else redirect("/maximus?acces=refuse");
  }
