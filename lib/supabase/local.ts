...
export function createLocalClient(){return{from:(table:string)=>new Query(table),auth:{getUser:async()=>({data:{user:typeof window!=="undefined"&&localStorage.getItem("nutvita-local-admin")==="1"?currentAdmin():typeof window!=="undefined"&&localStorage.getItem("nutvita-local-partner")==="1"?localPartnerUser:typeof window!=="undefined"&&localStorage.getItem("nutvita-local-client")==="1"?localClientUser:null},error:null}),onAuthStateChange:()=>({data:{subscription:{unsubscribe:()=>{}}}}),signOut:async()=>{if(typeof window!=="undefined"){localStorage.removeItem("nutvita-local-admin");localStorage.removeItem("nutvita-local-admin-email");localStorage.removeItem("nutvita-local-client");localStorage.removeItem("nutvita-local-partner")}await Promise.all([fetch('/api/admin/local-logout',{method:'POST'}),fetch('/api/client/local-logout',{method:'POST'}),fetch('/api/partner/local-logout',{method:'POST'})]);return{error:null}},signInWithPassword:async()=>({data:{user:null},error:new Error("Utilisez la connexion locale.")})},storage:{from:()=>({createSignedUrl:async()=>({data:null,error:new Error("Document indisponible en mode local.")}),upload:async()=>({data:null,error:null})})}} as any}
